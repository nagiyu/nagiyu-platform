...
describe('clipboard', () => {
  describe('readFromClipboard', () => {
    ...
    it('異常系: 権限エラー時に適切なエラーがスローされる', async () => {
      ...
      await expect(readFromClipboard()).rejects.toThrow('クリップボードの読み取りに失敗しました。手動で貼り付けてください。');
    });
  });
  ...
  describe('writeToClipboard', () => {
    ...
    it('異常系: 権限エラー時に適切なエラーがスローされる', async () => {
      ...
      await expect(writeToClipboard('test')).rejects.toThrow('クリップボードへの書き込みに失敗しました。');
    });
  });
});
