...
test.describe('MigrationDialog', () => {
  ...
  test.describe('基本動作', () => {
    test('初回訪問時にダイアログが表示される', async ({ page }) => {
      ...
      // タイトルが正しく表示される
      await expect(page.getByRole('heading', { name: 'Toolsアプリが新しくなりました' })).toBeVisible();
      
      // 本文が表示される
      await expect(page.getByText('このアプリは以前のバージョンから大幅にアップデートされました。')).toBeVisible();
      ...
      // プライバシー情報が表示される
      await expect(page.getByText('データはすべて端末内に保存されており、外部に送信されることはありません')).toBeVisible();
    });
    ...
    test('背景クリックでは閉じない', async ({ page }) => {
      ...
      await page.locator('[role="presentation"]').first().click({ position: { x: 1, y: 1 } });
