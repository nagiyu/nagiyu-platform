import { MOCK_SUMMARY_BY_PERIOD } from '../../../../lib/prediction-evaluation/mock-data';
...
describe('予測精度 PoC モックデータ', () => {
  describe('MOCK_SUMMARY_BY_PERIOD', () => {
    ...
    it('all 期間は空状態（judgedCount=0、dailyTrend=[]、bySignal の count はすべて 0）', () => {
      ...
      expect(all.bySignal.every((entry) => entry.count === 0)).toBe(true);
    });

    it('7d は dailyTrend 内に部分欠損（accuracy=null）を含む', () => {
