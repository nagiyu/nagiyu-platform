/**
 * ExchangeRepository 契約テスト（実行エントリポイント）
 *
 * InMemory実装と実DynamoDB実装（DynamoDB Local）の双方に対して
 * exchange.repository.contract.ts の共有スペックを実行する。
 *
...
import { defineExchangeRepositoryContract } from './exchange.repository.contract.js';
...
defineExchangeRepositoryContract('InMemory', {
  makeRepository: async () => new InMemoryExchangeRepository(inMemoryStore),
  makeTickerRepository: async () => new InMemoryTickerRepository(inMemoryStore),
  reset: async () => {
...
const dynamoDbLocalTableName = `contract-exchange-${process.pid}-${Date.now()}`;
...
defineExchangeRepositoryContract('DynamoDB Local', {
