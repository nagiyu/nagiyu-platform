/**
 * AlertRepository 契約テスト（実行エントリポイント）
 *
 * InMemory実装と実DynamoDB実装（DynamoDB Local）の双方に対して
 * alert.repository.contract.ts の共有スペックを実行する。
 *
...
import { defineAlertRepositoryContract } from './alert.repository.contract.js';
...
defineAlertRepositoryContract('InMemory', {
  makeRepository: async () => new InMemoryAlertRepository(inMemoryStore),
  reset: async () => {
...
const dynamoDbLocalTableName = `contract-alert-${process.pid}-${Date.now()}`;
...
defineAlertRepositoryContract('DynamoDB Local', {
  makeRepository: async () => {
    await ensureDynamoDbLocalTable();
    return new DynamoDBAlertRepository(
