...
export class ThreeBlackCrows extends CandlestickPattern {
  ...
  public readonly definition: PatternDefinition = {
    ...
    name: '黒三兵',
    description:
      '売りシグナル。3本の連続する陰線で構成され、下落トレンドへの転換または継続を示す。',
    signalType: 'SELL',
  };
  ...
  public analyze(candles: ChartDataPoint[]): PatternStatus {
    ...
    const condition1 =
      c2.close < c2.open && c2Body > c2Range * ThreeBlackCrows.LARGE_BODY_THRESHOLD;

    const c1Body = Math.abs(c1.close - c1.open);
    const c1Range = Math.abs(c1.high - c1.low);
    const condition2 =
      c1.close < c1.open &&
      c1Body > c1Range * ThreeBlackCrows.LARGE_BODY_THRESHOLD &&
      c1.open < c2.open &&
      c1.open > c2.close &&
      c1.close < c2.close;
    ...
    const condition3 =
      c0.close < c0.open &&
      c0Body > c0Range * ThreeBlackCrows.LARGE_BODY_THRESHOLD &&
      c0.open < c1.open &&
      c0.open > c1.close &&
      c0.close < c1.close;
