...
ここまでは一般的な実装例ですが、nagiyu-platform の Quick Clip 動画アップロードでは、本文のサンプルとは少し違う実数で運用しています。自分が一番こだわったのはサイズ別の戦略です。ファイルサイズが 100 MB 未満なら単純な `PutObject` の Presigned URL を 1 本返し（有効期限 1 時間）、100 MB 以上ならマルチパートアップロードに切り替えます。マルチパートでは 50 MB ごとにパート数を計算し、`UploadPart` の Presigned URL をパート数ぶん一括発行してクライアントに並列 PUT させます。上限は 20 GB、Content-Type は `video/mp4` 固定で、拡張子も `.mp4` をサーバー側で弾いています。動画は途中で回線が切れると痛いので、大きいファイルほどマルチパートで部分再送できるようにしたかった、というのが設計の動機です。

## 現在の運用

ダウンロード側は用途ごとに有効期限を変えています。生成済みの見どころクリップを個別にプレビューする URL は `GetObject` で 1 時間（3600 秒）、採用クリップをまとめた ZIP の一括ダウンロード URL は 5 分（300 秒）と、短命にしてあります。ZIP は重いので即時には存在せず、まず `HeadObject` で生成済みかを確認し、無ければ 202 PROCESSING を返して Lambda 非同期生成に回す、という二段構えにしました。本文では「アップロード用は 5 分程度」と書きましたが、弊プラットフォームではモバイル回線で大容量 mp4 を上げる前提があるので、アップロードは 1 時間と長めに取っています。期限はセキュリティとユーザー体験のトレードオフで、実際の回線と処理時間から逆算して決めるのが自分の結論です。
...
- **`Content-Type` の差異**: 署名に含めた値と PUT 時のヘッダが一致しないと 403。Quick Clip では `video/mp4` で固定しているので、クライアント側のヘッダも必ず合わせています。
