...
ここまでは一般的な実装例ですが、個人開発で運用しているある動画クリップ生成サービスの動画アップロードでは、本文のサンプルとは少し違う実数で運用しています。自分が一番こだわったのはサイズ別の戦略です。ファイルサイズが 100 MB 未満なら単純な `PutObject` の Presigned URL を 1 本返し（有効期限 1 時間）、100 MB 以上ならマルチパートアップロードに切り替えます。マルチパートでは 50 MB ごとにパート数を計算し、`UploadPart` の Presigned URL をパート数ぶん一括発行してクライアントに並列 PUT させます。上限は 20 GB、Content-Type は `video/mp4` 固定で、拡張子も `.mp4` をサーバー側で弾いています。動画は途中で回線が切れると痛いので、大きいファイルほどマルチパートで部分再送できるようにしたかった、というのが設計の動機です。
