...
- **新リリース時の HTML ページ**: 該当パスだけ Invalidate（`/about`, `/tech/*` など）
...
正直に書くと、運用しているサイトのルートドメインの CloudFront は、現時点ではこの記事で推奨したほどキャッシュを作り込めていません。`infra/root/cloudfront-stack.ts` を見ると、デフォルトビヘイビアは `CachingDisabled` で、ALB オリジンへほぼ素通しする構成になっています。これは Next.js（standalone）の SSR / SSG をまず確実に配信することを優先した結果で、CDN レイヤでの静的アセット分離キャッシュは「これからの宿題」として残しているのが実情です。

なので今は、CDN でガッツリ効かせるというより、アプリ側が返す `Cache-Control`（`_next/static` の `immutable` など）とブラウザキャッシュに寄せています。私自身、この記事で書いた「アセットは長く、HTML は短く、SSR は無効化」という理想形に対して、自分の実運用はまだ SSR 無効化の部分しか実装できていない、という温度感です。Information Gain の観点でも、こうした「理想と現状のギャップ」を正直に書けるのは実運用しているからこそだと思っています。
