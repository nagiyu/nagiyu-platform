...
そう判断してしまった理由は 2 つある。どちらも、たぶん多くの人が同じように踏む。

**1 つ目は、メールが英語だったこと。** 件名と、キー ID らしき文字列が並んでいるところだけを拾って、読むのを途中で諦めた。深刻度を自分で低く見積もった。

**2 つ目は、そのメールが 1 通ではなく、5〜6 通まとめて届いたこと。** これが効いた。
...
英語で、件名がほぼ同じものが束で並んでいる。この見た目になると、**「同じ内容の重複だろう、1 通読めば十分だろう」と処理してしまう。** 実際には、私はどれも最後まで読んでいない。
...
件数が多いことは、深刻度が低い証拠ではない。**むしろ逆で、件数はそのまま「漏洩したキーの本数」を意味していた。** 束で届いたら、まず何通あるかを数えて、全部開いたほうがいい。
