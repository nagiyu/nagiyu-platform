...
私が主に想定しているのは、nagiyu プラットフォームの各サービス（Quick Clip・Codec Converter・Stock Tracker など）を日常的に使うユーザーです。「サービスを使いたいだけなのに認証の設定で詰まりたくない」という人に向けて、Google アカウントを持っていればすぐ使えるよう設計しています。また、nagiyu の API を組み合わせて自分のアプリケーションを作りたい開発者や、プラットフォーム上のユーザーを管理するシステム管理者も主要な利用者として想定しています。一方で、Google アカウントを持っていないユーザーは現時点では利用できません。複数の OAuth プロバイダーへの対応は将来の課題として認識しており、Phase 2 以降で検討しています。
