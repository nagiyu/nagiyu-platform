...
/** EventID として辞書順ソート可能なゼロ埋め文字列を作る（ULID の代わりに決定的な順序で検証するため） */
function paddedEventId(n: number): string {
...
export function defineSafetyEventRepositoryContract(
  ...
  describe(`SafetyEventRepository 契約: ${label}`, () => {
    ...
    it('create したデータを getById で取得できる', async () => {
      const input = buildSafetyEventInput({ EventID: paddedEventId(1) });
    ...
    it('listRecent は EventID（ULID）降順で返す（挿入順ではない）', async () => {
      ...
      await repository.create(buildSafetyEventInput({ UserID: 'u-a', EventID: paddedEventId(3) }));
      await repository.create(buildSafetyEventInput({ UserID: 'u-b', EventID: paddedEventId(1) }));
      await repository.create(buildSafetyEventInput({ UserID: 'u-c', EventID: paddedEventId(2) }));

      const result = await repository.listRecent(10);

      expect(result.map((r) => r.EventID)).toEqual([
        paddedEventId(3),
        paddedEventId(2),
        paddedEventId(1),
      ]);
    ...
    it('listRecent は limit で件数を絞り、最近（EventID降順で先頭）のものを返す', async () => {
      for (let i = 1; i <= 5; i += 1) {
        await repository.create(
          buildSafetyEventInput({ UserID: `u-${i}`, EventID: paddedEventId(i) })
        );
      }

      const result = await repository.listRecent(2);

      expect(result.map((r) => r.EventID)).toEqual([paddedEventId(5), paddedEventId(4)]);
    });
    ...
    it('listRecent は100件超のイベントでも最近の検出を取りこぼさない', async () => {
      ...
      const expectedEventIds = Array.from({ length: 50 }, (_, i) => paddedEventId(total - i));
