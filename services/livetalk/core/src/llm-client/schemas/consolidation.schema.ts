...
export const ConsolidationSchema = z.object({
  topics: z.array(
    z.object({
      ...
      /**
       * この Topic の webFacts を「[依頼 #N]」と表示された Web 取得生データから作った場合、
       * その番号 N をすべて入れる（依頼由来 provenance を index 参照方式で紐付ける）。
       * 依頼由来でなければ空配列（[]）にする。原文の依頼文はコード側が権威ある WebRaw
       * エンティティから直接取るため、LLM は番号以外（依頼文の文字列など）を返さない。
       */
      sourceRequestIndices: z
        .array(z.number().int())
        .describe(
          'この Topic の webFacts を「[依頼 #N]」と表示された Web 取得生データから作った場合、その番号 N をすべて入れる。依頼由来でなければ空配列。プロンプトに表示された [依頼 #N] の N をそのまま使い、番号以外（依頼文の文字列など）は返さない。'
        ),
      /**
       * ユーザー自身についての新規事実。
