 ...
 * - `aborted`: ストリーミングが上記いずれの終了イベントにも到達しないまま終わった。
 *   消費側がループを `break` / `return` で中断した場合のほか、ストリーム途中で
 *   エラーが throw された場合も含む。いずれも真の usage は原理的に取得不可能なため、
 *   欠測したこと自体だけを記録する（Step 2 で「実測が全体の何割をカバーしているか」を
 *   出すための分母になる）
 *
