...
| オーケストレーション（再帰） | Opus | integration / develop | Issue から仕様を詰め、規模を見て「自分が末端か／さらに分岐か」を判断。実装の駆動・検証・dev 検証・宿題の切り出しを回す。**develop 反映後のドキュメント追従判断**まで見届ける |
...
3. 単一視点の漏れを補うため、オーケストの判断で **fresh-eyes レビュアー（Opus サブエージェント）**を起動してよい（→「fresh-eyes レビュー」）。起動可否はオーケストに一任する（超軽微は畳む）。
4. 妥当なら、**オーケストが** Draft PR を作成し、ウォッチする（CI 失敗の autofix を含む）。**PR 作成はサブエージェントにさせない。**
5. **実装単位（サブエージェント 1 回）ごとに小さい Draft PR + 人力レビュー**とする（大粒度レビューは避ける）。
6. ★ 人がレビュー・Ready 化・マージ ★（Claude は介入しない）。

### ドキュメント追従判断（develop 反映後）

実装資材が develop に取り込まれた後（develop 直 / integration 経由を問わず）、オーケストレーターは **最後に `docs/` への追従が必要かを判断する**。「クローズしておしまい」で永続ドキュメントが置き去りになるのを防ぐための工程。
...
| fresh-eyes レビュー（サブエージェント） | Opus | 妥当性の検証・判断が中心（見落とし防止） |
...
オーケストレーター単独の検証は**単一視点ゆえの漏れ**が起こりうる。これを補うため、オーケストレーターは [`.claude/agents/reviewer.md`](.claude/agents/reviewer.md)（Opus）を **fresh-eyes（白紙視点）レビュアー**として起動できる。最終の人力レビューでの手戻り・見落としを減らすのが狙い。

- **コンテキスト隔離**: レビュアーには**実装方針・implementer へのプロンプト・会話履歴を渡さない**。diff・成果物だけを別コンテキストで評価させ、先入観を排す（fresh-eyes 要件は別コンテキストで走るサブエージェントでしか満たせない）。
- **起動はオーケストレーターに一任**: 規模・リスクに応じて起動可否を判断する（超軽微なら畳んでよい）。客観信号検証（テスト / ビルド / diff）を**置き換えるものではなく補強**する。
- **役割境界**: レビュアーは**妥当性の評価のみ**を行い、指摘を「**クリティカル / 仕様確認要 / 軽微**」に区別して返す。修正・PR 作成・Issue 操作は行わない（いずれもオーケストレーターの責務）。
- **指摘の扱いと最終判断**: 最終判断はオーケストレーターが持つ。軽微・通常の指摘は**そのまま修正**してよいが、**クリティカル / 仕様確認要のものは人に指示を仰ぐ**（独断で大きく方針転換しない）。
