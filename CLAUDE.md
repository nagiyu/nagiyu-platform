...
- Issue 本文は **単独で実装に着手できる粒度** で書く（背景・スコープ・完了条件・実装方針・対象ファイル・依存関係・実装上の注意点）
- これは Sonnet 移譲・セッション自動圧縮・新規セッション切替への防御策を兼ねる
- 大規模対応で `tasks/{feature-name}/` を作成する場合も、進捗管理は Issue 側で行う（サブ Issue で分割可）
...
- **小規模対応**: `tasks/` ディレクトリは作成しない。Issue 本文に WHAT / WHY / HOW（実装方針・対象ファイル・依存関係・注意点）を集約する
...
コスト最適化のため、フェーズに応じてモデルを使い分ける。

### 推奨フロー（小規模対応）

1. **Opus**: Issue 起票 → integration ブランチ作成 → 必要に応じて作業ブランチでの初期計画
2. **Sonnet**: 同一セッションでモデルを切り替えて実装継続

### 前提条件

Sonnet が単独で読み解けるよう、**Issue 本文に WHAT / WHY / HOW を網羅する**。

| 区分 | 内容 |
|------|------|
| WHAT | スコープ・完了条件 |
| WHY | 背景・目的 |
| HOW | 実装方針・対象ファイル・依存関係・実装上の注意点 |

これはセッション自動圧縮（コンテキスト上限到達時）や、新規セッションへの切替に対する防御策としても機能する。

### 大規模対応

`tasks/{feature-name}/` の仕様ドキュメントが Sonnet への引き継ぎ書を兼ねる。具体的なモデル切替ポイントは Issue ごとに判断する。
