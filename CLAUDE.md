...
セッションを分ける目的は、親のコンテキストを守ることだけではない。**意思決定が人と交わる地点を分散させること**が本質である。

- 最上位で細かい設計判断まで決めてしまうと、そこだけコンテキストが肥大して詰む。**最上位は采配に徹する**。
- 子セッションで判断に迷ったら、子が**人に**聞く。親には聞かない。
- **これがサブエージェントとの決定的な違い**。サブエージェントは走り出したら人が介入できない。セッションなら入れる。
- 「人は最上位だけを見る」運用ではない。むしろ**人が各セッションへ降りていく**。

したがって「規模が大きいからセッションを分ける」ではなく、「**意思決定の場を増やしたいからセッションを分ける**」と理解する。物量だけが理由ならサブエージェントで足りる。
...
> 起動プロンプトには、**子が自力で辿り着けない情報だけ**を書く。

子は `CLAUDE.md` を自動でロードし、Issue もコードも自分で読める。厚さは問題にならない。問題になるのは、子が取得できず、かつ「足りない」ことにすら気づけない情報に限られる。

| 書くべきもの | 理由 |
|---|---|
| 親で人と合意済みで、まだ Issue に落ちていないこと | 子は親の会話履歴を持たない。Issue に無ければ**永久に知り得ず**、同じ論点を人ともう一度議論することになる |
| 自分のスコープの境界 | 知らないと隣の Phase まで手を出すか、自分の範囲でも判断を保留する |
| 人に聞くべき判断の種類 | 分散設計の肝。渡さないと勝手に走るか、止まりすぎるかに振れる |

加えて、プロンプトではなく **API パラメータ**として `source_url` / `source_revision` を渡す（integration 起点か develop 起点かは子には判別できない。未指定だとリポジトリ自体が clone されない）。

この鉄則は既存の思想と同じ形である。docs は「コードを読めばわかることは書かない」、Issue は「実装詳細を front-load しない」、起動プロンプトは「**子が読みに行けることは書かない**」。

なお、起動プロンプトが厚くなるほど **Issue が薄い証拠**になる、というチェックとしても機能する。
...
- **合意を得ずに子セッションを起動すること**（→「セッションの起動（再帰の実行）」。サブエージェントの spawn は対象外）
- **`fire_trigger`（Routine の手動即時発火）を追い指示に使うこと**（`persistent_session_id` を無視して別セッションを作り、モデルも継承されず、生成されたセッションは親から片付けられない）
- **PR のチェックイン予約・Routine（`send_later` / `create_trigger`）を自発的に作ること**（PR の変化は `subscribe_pr_activity` で届くもので足りる。定期的な発火は人へのレビュー催促になる。明示的に頼まれた場合を除く）
- **対応不要な PR イベントをチャットで逐一報告すること**（CI 通過の bot コメント等。対応したとき・人の判断が要るときだけ報告する）
...
セッションを分けて進める際の制約（親子間で何が届き何が届かないか・起動コスト・遅延）は [`docs/development/claude-sessions.md`](docs/development/claude-sessions.md) にまとめてある。
...
- セッション運用の実測と制約: [`docs/development/claude-sessions.md`](docs/development/claude-sessions.md)
