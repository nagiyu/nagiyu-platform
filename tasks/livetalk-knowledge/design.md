...
### 4.6 既知の制約・将来拡張（所見 C / D・移行運用）

本再設計で残した「静観・後回し」の判断を、セッションの継ぎ目をまたいで残すための記録。いずれも実装しておらず、実害が積み上がった時点で着手を判断する。

**所見C: Topic ルーティング / マージ品質（近似重複の吸収不足＝過分割）**

- §4.3 で「ルーティング/マージ品質が律速」と挙げた点の実測。consolidation の埋め込み近傍＋LLM のマージ判断が、近い素材を 1 つの Topic に畳みきれず割れることがある。
- dev 実測: 移行（冗長な旧 Knowledge の一括投入）で「ラーメン ×4」「AI/プライバシー ×7」等の近似重複 Topic が発生。冗長な旧データほど顕在化し、care も分散させる。
- 移行固有ではなく consolidation 本体の性質。prod は旧データが小さく冗長も少ないため影響は軽い見込み。
- care 意味論はレバー1（§4.5）で「ユーザー起点」化済み。care の自然減衰はレバー2（§4.5 の将来拡張）。
- 将来: ルーティング/マージ閾値の改善、または近似 Topic の merge/dedup パス。現状は静観。

**所見D: 想起が汎用発話に弱い**

- 現状の想起は (1)「今の 1 発話だけ」を embed し、(2) care を使わない（関連度 only）。
- dev 実測: 「いいゲームないかな」のような汎用発話は具体名の Topic（例「スプラトゥーン レイダース」）と cosine が閾値（`TOPIC_RECALL_SIMILARITY_THRESHOLD = 0.4`）に届かず想起されない（`retrievedTopicCount = 0`）。「ラーメン食べたい」は語が一致して想起される。
- (1) は requirements UC-002「発話＋直近文脈を埋め込む」との実装乖離（＝直せるギャップ）。(2) の care 不使用は意図的（"コーヒー病" 回避、§4.3 / §2.4）。
- 将来: 案2（想起クエリに直近数ターンを含めて embed ＝仕様どおりに戻す）／案3（cosine がニアミス帯＝閾値の少し下のときだけ care で後押し）。会話想起は関連度 only で自然に薄れる利点があるため、care 後押しは過剰想起の再来リスクに注意。現状は静観。

**移行運用メモ（一回性 Lambda・throwaway）**

- **care-seeder**: 旧 InterestCategory の Weight を新 Topic の care に載せる際、カテゴリ名の文字列一致で探すと、移行後 Topic の Subject が LLM で言い換えられるため全 drop する（dev 実測）。→ InterestCategory.Embedding と Topic.Embedding の近傍マッチへ修正する。
- **実行時間**: Lambda 上限 15 分に対し、dev は旧データが多く（Knowledge 372 / Memory 145）全キャラ一括だと超過する。**キャラ単位で実行**すれば収まる。prod は小規模（Knowledge 67 等）で問題なし。
- **同時実行**: 破壊的 one-shot に多重起動ガードは無い。同期呼び出しはブラウザ側が待てず「Load failed」を返すが Lambda 本体は裏で完走するため、それを見て再発火すると wipe + migrate が交錯してデータが散らかる。**非同期 (Event) で 1 回だけ発火し、CloudWatch で完走（「バッチ完了」）を確認**する運用でカバーする。
...
- [ ] `docs/services/livetalk/architecture.md` に ADR として追記：
      ...
      - care 上昇はユーザー起点でのみ（レバー1、§4.5）。自然減衰（レバー2）は将来拡張
      - ルーティング/マージ品質が律速 → provenance で可逆化・条件付き書込
- [ ] §4.6「既知の制約・将来拡張（所見 C / D・移行運用）」を `architecture.md` の「既知の制約」節へ移す（過分割＝ルーティング品質、想起の汎用発話弱・UC-002 乖離、移行運用の注意）。静観継続の判断が継ぎ目をまたぐため、docs に永続化する。
