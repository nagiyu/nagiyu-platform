...
- 標準化するには tsconfig の `"rootDir"` を `"."` に変更する必要がある（変更後は `src/index.ts` → `dist/src/index.js`）
...
- tsconfig.json に `rootDir` 未指定（`include: ["src/**/*"]` のため実効値は `./src`）かつ `"outDir": "./dist"` のため、`src/index.ts` → `dist/index.js`
...
- **修正**: `package.json` の `main` → `dist/src/index.js`、`types` → `dist/src/index.d.ts` に変更し、tsconfig の `rootDir` を `"."` に設定して出力先を `dist/src/` に揃える
...
- T013・T014 は、T004 の調査結果をもとに実施すること。
  - `niconico-mylist-assistant-core` の修正は `package.json` のパス修正に加え、tsconfig の `rootDir` を `"."` に設定して `dist/src/` 出力に統一する。
  - `stock-tracker-core` の修正は tsconfig の `rootDir` を `"."` に変更して `dist/src/` 出力に統一する必要があり、`web/next.config.ts` の `transpilePackages` を使ったランタイム動作に影響するため CI での確認が必須。
