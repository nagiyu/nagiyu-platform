...
- [x] **T001**: `core/src/db/videos.ts`の`listVideosWithSettings`関数を修正
    - `limit`パラメータのデフォルト値を`undefined`に変更（未指定時は全件取得）
    - 既存の呼び出し元（`/api/videos/route.ts`）への影響を確認
    - テストコードで`limit`未指定のケースの挙動変更を確認
- [x] **T002**: `core/src/db/videos.ts`に`selectRandomVideos`関数を追加
    - フィルタ条件を受け取り、フィルタ後の全動画を取得（`limit`未指定で呼び出し）
    - Reservoir Samplingでランダムに`maxCount`件を選択
    - 選択した動画を返す
- [x] **T003**: `/api/mylist/register/route.ts`を修正
...
- [x] **T005**: `core/tests/unit/list-videos.test.ts`を更新
    - `limit`未指定時の挙動変更に対応するテストケースを追加
- [x] **T006**: `core/tests/unit/select-random-videos.test.ts`を作成
...
- [x] DynamoDBの総件数が100件未満の場合、正常に動作する
- [x] DynamoDBの総件数が100件ちょうどの場合、正常に動作する
- [x] DynamoDBの総件数が100件を超える場合（例：150件）でも、正常に`maxCount`件（例：50件）を選択できる
- [x] フィルタ条件（お気に入りのみ、スキップ除外）が正しく適用される
- [x] フィルタ後の動画数が0件の場合、適切なエラーメッセージが表示される
- [x] テストカバレッジが80%以上である
- [x] 既存のE2Eテストが引き続き成功する
- [x] コードレビューで指摘された問題がすべて解決されている
- [x] セキュリティスキャンで脆弱性が検出されていない、または修正済みである
...
- 完了日: 2026-02-17
- 追加知見:
    - `limit` 未指定時の全件取得を許容しても、ページネーション用途の呼び出しは件数指定を明示する運用で整合を維持できる
    - 一括登録の抽出責務を独立させることで、一覧取得の責務を維持したまま抽出ロジックを差し替え可能にできる
