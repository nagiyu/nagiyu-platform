...
- [ ] T008: `services/auth/web/Dockerfile` の `FROM node:22-alpine` → `node:24-alpine` に変更する（依存: なし）
- [ ] T009: `services/codec-converter/batch/Dockerfile` の `FROM node:22-slim` → `node:24-slim` に変更する（依存: なし）
- [ ] T010: `services/codec-converter/web/Dockerfile` の `FROM node:22-alpine` → `node:24-alpine` に変更する（依存: なし）
- [ ] T011: `services/stock-tracker/batch/Dockerfile` の `FROM public.ecr.aws/lambda/nodejs:22` → `nodejs:24` に変更する（依存: なし）
- [ ] T012: `services/stock-tracker/web/Dockerfile` の `FROM node:22-alpine` → `node:24-alpine` に変更する（依存: なし）
- [ ] T013: `services/share-together/web/Dockerfile` の `FROM node:22-alpine` → `node:24-alpine` に変更する（依存: なし）
- [ ] T014: `services/admin/web/Dockerfile` の `FROM node:22-alpine` → `node:24-alpine` に変更する（依存: なし）
- [ ] T015: `services/niconico-mylist-assistant/web/Dockerfile` の `FROM node:22-alpine` → `node:24-alpine` に変更する（依存: なし）
- [ ] T016: `services/tools/Dockerfile` の `FROM node:22-alpine` → `node:24-alpine` に変更する（依存: なし）
...
- [ ] T021: `npm run lint` を実行し、Lint エラーがないことを確認する（依存: T004 または T003）
- [ ] T022: `npm test` を実行し、既存テストがすべてパスすることを確認する（依存: T004 または T003）
- [ ] T023: `npm audit` を実行し、新たな脆弱性がないことを確認する（依存: T002）
