...
- [x] T021: `npm audit` を実行し、新たな脆弱性がないことを確認する（依存: T002）
    - 実行結果（2026-03-23）: 3件（high: 2 / moderate: 1）を検出。`fast-xml-parser` / `flatted` の既知脆弱性であり、今回の Node.js v24・`@types/node` v24 対応で新規導入された依存は確認されなかった。
...
- [ ] 既存テストがすべてパスしている
- [x] `npm audit` を実行し、既知脆弱性の内容と影響範囲を確認した
