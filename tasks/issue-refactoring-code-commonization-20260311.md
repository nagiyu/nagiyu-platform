...
- [x] T001: `libs/nextjs/src/types/next-auth.d.ts` を新規作成し、4 サービスの next-auth.d.ts を統合
    - 影響ファイル: 上記 1.2 の 4 ファイルを削除または空の再エクスポートに置き換え
- [x] T002: `libs/nextjs/src/auth-config.ts` を新規作成し、cookieOptions / cookies / session の共通部分をファクトリ関数として提供
    - 影響ファイル: auth.ts 系 4 ファイルをリファクタリング
- [x] T003: `libs/aws/src/dynamodb/client.ts` に `getDynamoDBDocumentClient()` / `getTableName()` / `clearDynamoDBClientCache()` を追加
    - デフォルトリージョンは `us-east-1` に統一（CloudFront 対応のため）
    - 影響ファイル: aws-clients.ts 系 4 ファイル、dynamodb-client.ts

### フェーズ 2: 中優先度・利用率向上

- [x] T004: `libs/nextjs/src/health.ts` を新規作成し、`createHealthRoute()` を提供
    - 影響ファイル: 6 サービスの `api/health/route.ts`
- [x] T005: `@nagiyu/nextjs` の利用を niconico-mylist-assistant へ拡大
...
- [x] T006: `services/admin/web/src/types/` の独自 User/Session 型を整理（T001 完了後）
    - `@nagiyu/nextjs` の next-auth.d.ts 型で代替可能な定義を削除
- [x] T011: codec-converter に `@nagiyu/common` 依存を追加
    - `services/codec-converter/web/package.json` に `@nagiyu/common` を追加し、共通型・ユーティリティを利用可能にする
...
- **Session 型の二重管理（ベストプラクティス）**: 2 つの Session 型を以下の役割で明確分離する。
    - **`@nagiyu/common` の `Session` 型**: DynamoDB ストア上の完全エンティティ（createdAt/updatedAt/lastLoginAt 含む）。
      サーバーサイドのリポジトリ層・バッチ処理のみが参照する。
    - **`libs/nextjs/src/types/next-auth.d.ts` の Session 拡張**: NextAuth の JWT/セッショントークンから
      復元される軽量な Web セッション表現（id, email, name, roles のみ）。フロントエンド・API ルートが参照する。
    - `getSession()` の戻り型には `@nagiyu/common` の Session を使わず、NextAuth の Session 拡張型を使う。
      createdAt/updatedAt が必要なサーバーサイド処理は DynamoDB を直接参照する設計とする。

- **codec-converter への `@nagiyu/common` 依存追加**: codec-converter は `@nagiyu/common` を
  依存に含めていない（package.json 参照）。共通化推進にあわせて追加が必要。→ T011 としてタスク化済み。

- **デフォルトリージョンの統一方針決定済み**: DynamoDB クライアントのデフォルトリージョンは
  **`us-east-1` に統一する**（CloudFront との整合を取るため）。
  `libs/aws` のファクトリ関数では `region: 'us-east-1'` をデフォルト値とし、
  `ap-northeast-1` を使用していたサービス（niconico/core, stock-tracker/batch）は T003 完了時に合わせて修正する。
