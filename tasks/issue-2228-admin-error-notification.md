...
CloudWatch Alarms が検知したエラーを、Admin サービスをインストールしている端末へ
...
- Admin PWA から通知許可を取得し、インストール済みの端末にプッシュ通知を届ける
...
- `SubscriptionConfirmation`: SNS が POST で送ってくる `SubscribeURL` に対して
  ...
  NFR2 に従い 5 秒以内に完了するようタイムアウトを設定すること（例: 4000ms）。
  タイムアウトや HTTP エラー時はログ出力して 500 を返す。リトライは SNS 側が自動で行う。
...
| 属性名       | 型     | 説明                                |
| ------------ | ------ | ----------------------------------- |
| UserId       | String | パーティションキー（Auth サービスの JWT から取得するユーザー ID。Admin の `src/lib/auth/session.ts` の `getSession()` 経由で参照）|
...
- **SNS サブスクリプション登録方法とデプロイ順序**: CDK `UrlSubscription` での自動登録を推奨する（IaC として一元管理できるため）。
  ただし、CDK デプロイ時に SNS が Admin エンドポイントへ SubscriptionConfirmation を送るため、
  T010 の実装とデプロイが完了してから SNS スタック（T001）に `UrlSubscription` を追加してデプロイする必要がある。
  このため、**Phase 1 の T001 では SNS トピックの作成のみを行い、UrlSubscription の追加は Phase 4 の T015 で実施する**。
  Phase 1 → Phase 2/3 → Phase 4 の順に実施することを厳守すること。
  初回セットアップでは SNS コンソールから手動登録し確認後に CDK 管理へ移行する方法も許容する。
