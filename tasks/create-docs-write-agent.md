...
- FR6: エージェントが作成するMarkdownのインデントはスペース4つを厳守する（`.vscode/settings.json` 準拠）
- FR7: エージェント完成後、使用方法を `docs/agents/` 配下にREADMEとして作成する

### 非機能要件

- NFR1: 実装（コード生成・変更）は行わない。ドキュメント作成に専念する
- NFR2: エージェント定義は既存エージェント（task.implement, task.proposal）のパターンに準拠する
