...
|--------------|------|--------------------------|
| 予測採点結果 | AI 予測 1 件あたりの採点結果 | 銘柄、日付、予測シグナル、実績リターン、Hit/Miss、採点タイムスタンプ |
| 既存：日次サマリー | AI 解析と OHLCV を保持（既存） | 銘柄、日付、OHLCV、AI 解析結果、AI 解析エラー |
...
erDiagram
    ...
    DailySummary ||--o| PredictionOutcome : "採点される (1:0..1)"
    DailySummary {
        string TickerID
        date Date
        decimal Close
...
物理 DB スキーマ（PK/SK・GSI 設計）は [`design.md`](./design.md) を参照。
