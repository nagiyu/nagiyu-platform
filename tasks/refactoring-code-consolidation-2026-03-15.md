...
- `@nagiyu/common` に `sendWebPushNotification(subscription: PushSubscription, payload, vapidConfig)` を Node.js 専用サブエントリとして追加する
...
- 各バッチサービスは `@nagiyu/common` の共通関数を呼び出す薄いラッパーになる
...
- [ ] T008: Web Push 通知送信の共通関数を `@nagiyu/common/src/push/web-push.ts` に切り出し
    - 関数: `sendWebPushNotification(subscription: PushSubscription, payload, vapidConfig)`
...
- [ ] T009: `@nagiyu/nextjs` に `createPushSubscribeRoute(options)` を追加
...
- [ ] T010: admin, auth, codec-converter, share-together, niconico の `ThemeRegistry.tsx` を削除し、各 `layout.tsx` から `ServiceLayout` を直接呼び出す形に変更
    - 対象ファイルの削除: 各サービスの `ThemeRegistry.tsx`
    - 各 `layout.tsx` で `ServiceLayout` に title/ariaLabel/headerSlot を直接渡す
- [ ] T011: `services/tools/src/components/ThemeRegistry.tsx` を `ServiceLayout` ベースに移行（独立した課題として対応）

### フェーズ 5: Repository Factory 共通化（`@nagiyu/aws`）

- [ ] T012: `@nagiyu/aws` にジェネリックなファクトリーヘルパー `createRepositoryFactory<T>` を追加
...
4. **stock-tracker Alert エンティティの変更**: `SubscriptionEndpoint` 等フラットフィールドから `subscription: PushSubscription` への移行は
