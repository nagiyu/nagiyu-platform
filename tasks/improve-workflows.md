...
- FR1: Admin、Tools、Codec Converter に段階的フロー（lint/format-check → build → test → E2E）を適用する
...
### Phase 1: 段階的フローの適用（Admin/Tools/Codec Converter）

- [ ] T001: Admin の Fast/Full ワークフローを調査し、段階的フロー（lint → build → test → e2e）を整理する
- [ ] T002: Tools の Fast/Full ワークフローを調査し、段階的フロー（lint → build → test → e2e）を整理する
- [ ] T003: Codec Converter の Fast/Full ワークフローを調査し、段階的フロー（lint → build → test → e2e）を整理する

### Phase 2: E2E テストのデバイス並列化

- [ ] T004: Stock Tracker の E2E ジョブをデバイスごとに分割する（chromium-mobile / chromium-desktop / webkit-mobile）
- [ ] T005: Niconico Mylist Assistant の E2E ジョブをデバイスごとに分割する
- [ ] T006: Admin の E2E ジョブをデバイスごとに分割する
- [ ] T007: Tools の E2E ジョブをデバイスごとに分割する
- [ ] T008: Codec Converter の E2E ジョブをデバイスごとに分割する

### Phase 3: Fast/Full ワークフローの統合

- [ ] T009: Admin の fast/full を `admin-verify.yml` に統合し、旧ファイルを削除する
- [ ] T010: Tools の fast/full を `tools-verify.yml` に統合し、旧ファイルを削除する
- [ ] T011: Codec Converter の fast/full を `codec-converter-verify.yml` に統合し、旧ファイルを削除する
- [ ] T012: Stock Tracker の fast/full を `stock-tracker-verify.yml` に統合し、旧ファイルを削除する
- [ ] T013: Niconico Mylist Assistant の fast/full を `niconico-mylist-assistant-verify.yml` に統合し、旧ファイルを削除する

### Phase 4: 動作確認

- [ ] T014: `integration/**` ブランチへのPRで chromium-mobile のみの E2E が実行されることを確認する
- [ ] T015: `develop` ブランチへのPRで全デバイスの E2E が並列実行されることを確認する
- [ ] T016: `develop` へのPRでカバレッジチェックが実行されることを確認する
- [ ] T017: 全サービスの段階的フローが正しく機能することを確認する（lint 失敗時にビルドがスキップされること等）
...
- auth-verify.yml は E2E テストを持たないインフラ専用ワークフローのため対象外
