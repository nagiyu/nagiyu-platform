...
- [x] T011: `@types/jest` を各ワークスペース（libs/common, libs/react, libs/nextjs, stock-tracker/web, infra/codec-converter）から削除（ルートに集約済み）
- [x] T012: `@types/node` を各ワークスペース（libs/aws, libs/react, stock-tracker/core, infra/codec-converter）から削除（ルートに集約済み）
- [x] T013: `@types/web-push` のルート統合を検討・実施
