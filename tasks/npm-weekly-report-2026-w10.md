...
- [ ] T001: ルート `package.json` に `overrides` セクションを追加し `serialize-javascript >= 7.0.3` を設定
- [ ] T002: ルート `package.json` に `overrides` セクションを追加し `fast-xml-parser >= 5.3.8` を設定
- [ ] T003: ルートから `npm install` を実行して `package-lock.json` を更新
- [ ] T004: `npm audit` を再実行して脆弱性が解消されたことを確認

### Phase 2: パッケージ更新（パッチ・マイナー）

- [ ] T005: ルートの AWS SDK 系パッケージを更新（`@aws-sdk/*` を 3.1004.0 へ）
- [ ] T006: ルートの MUI 系パッケージを更新（`@mui/*` を 7.3.9 へ）
- [ ] T007: `@types/node` をルートと全ワークスペースで `^22.19.15` へ更新
- [ ] T008: 各ワークスペースの AWS SDK パッケージを更新
- [ ] T009: その他のパッチ更新（eslint, typescript-eslint, aws-cdk, openai, playwright, tailwindcss など）
- [ ] T010: ビルドおよびユニットテストで動作確認
