...
以下のサービスで verify と deploy の Docker ビルドが乖離しているため、対応が必要。

| サービス                  | verify（Docker ビルド）                | deploy（Docker ビルド）                | 差異の内容                       |
| ------------------------- | -------------------------------------- | -------------------------------------- | -------------------------------- |
...
| share-together            | `share-together-verify.yml`            | `share-together-deploy.yml`            | `--build-arg APP_VERSION` の有無 |
| admin                     | `admin-verify.yml`                     | `admin-deploy.yml`                     | `--build-arg APP_VERSION` の有無 |
| auth                      | `auth-verify-app.yml`                  | `auth-deploy.yml`                      | `--build-arg APP_VERSION` の有無 |
...
- [x] T001: `.github/actions/build-docker-image/action.yml` を作成する（lock 機構を内包、`app-version` input を持つ）
- [x] T002: `stock-tracker-verify.yml` の docker-build-web / docker-build-batch を `build-docker-image` に置き換え、`app-version` を渡す
- [x] T003: `stock-tracker-deploy.yml` の Docker ビルド部分を `build-docker-image` に置き換える（プッシュは呼び出し元で継続）
- [x] T004: `codec-converter` の verify / deploy で同様に書き換える
- [x] T005: `niconico-mylist-assistant` の verify / deploy で同様に書き換える
- [x] T006: `tools` の verify / deploy で同様に書き換える
- [x] T007: `admin-verify.yml` / `admin-deploy.yml` で同様に書き換える
- [x] T008: `auth-verify-app.yml` / `auth-deploy.yml` で同様に書き換える
- [x] T009: `share-together` の verify / deploy で同様に書き換える

### Phase 2: Web ビルドの verify/deploy 間統一

- [x] T010: `.github/actions/build-web-app/action.yml` を作成する
- [x] T011: 各サービスの verify / deploy の Web ビルドステップを `build-web-app` に置き換える

### Phase 3: その他の共通化（サービス横断）

- [x] T012: `.github/actions/setup-node/action.yml` を作成する（Node.js セットアップ + `npm ci`）
- [x] T013: 全ワークフローの Node.js セットアップを `setup-node` に書き換える
- [x] T014: `.github/actions/setup-environment/action.yml` を作成する（dev/prod 環境判定）
- [x] T015: deploy 系ワークフローの環境判定を `setup-environment` に書き換える
...
- `auth-verify.yml` は Cognito / インフラ構成（CDK）の検証ワークフローであり、Docker ビルドを含まないため対象外とする
