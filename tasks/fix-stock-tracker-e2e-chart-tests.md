...
-   FR2: 使用する取引所キーは TradingView API で有効なもの（例: `NASDAQ`）であること
    -   TradingView の取引所キーは `NASDAQ`（コードベースの `types.ts` 等では `NSDQ` と記載されているが、TradingView 実環境では `NASDAQ` が正しいキー）
...
-   `TestDataFactory.createExchange({ key: 'NASDAQ', name: 'NASDAQ Stock Market' })` で実在する取引所を作成
-   `TestDataFactory.createTicker({ symbol: 'NVDA', name: 'NVIDIA Corporation', exchangeId: ... })` で実在するティッカーを作成
-   `NASDAQ:NVDA`（NVIDIA）は流動性が高く、TradingView API で常にデータが取得できる
...
-   他のテスト（`top-page-selector.spec.ts` 等）は架空のティッカーを使用しているが、
    チャート描画を期待しないため修正不要
-   `NASDAQ:NVDA` が将来的に廃止・上場廃止になった場合は、別の実在銘柄に変更が必要
-   コードベースの `types.ts` や `validation/index.ts` 等では `NSDQ` が NASDAQ のキーとして
    記載されているが、TradingView 実環境では `NASDAQ` が正しいキー。
    コアコード側の修正は本タスクのスコープ外（別途検討が必要）
