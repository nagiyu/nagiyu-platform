...
- [x] T006: `libs/nextjs/src/error.ts` に `createErrorResponse(status, error, message)` 関数を export として追加する（依存: T001）
- [x] T007: `libs/nextjs/src/index.ts` に `createErrorResponse` を再 export する（未 export の場合のみ）（依存: T006）
- [x] T008: `services/admin/web/src/app/api/notify/subscribe/route.ts` のローカル `createErrorResponse()` を削除し、`@nagiyu/nextjs` から import する形に変更する（依存: T007）
- [x] T009: `services/admin/web/src/app/api/notify/sns/route.ts` のローカル `createErrorResponse()` を削除し、`@nagiyu/nextjs` から import する形に変更する（依存: T007）
- [x] T010: `libs/nextjs` のユニットテストを追加して `createErrorResponse()` の動作を検証する（依存: T006）
- [x] T011: admin/web のビルドとテストを実行して動作を確認する（依存: T008, T009, T010）

## Phase 3: Web Push ラッパーの除去（優先度: 高）

<!-- 両サービスの sendNotification() ラッパーを除去し、sendWebPushNotification() を直接呼び出す形に統一する -->

- [x] T012: niconico-mylist-assistant/batch と stock-tracker/batch の VAPID subject を `mailto:support@nagiyu.com` にプロジェクト共通でハードコード統一する（依存: なし）
- [x] T013: `services/niconico-mylist-assistant/batch/src/lib/web-push-client.ts` の `sendNotification()` を削除し、ペイロード生成関数のみ残す（依存: T012）
- [x] T014: `services/niconico-mylist-assistant/batch/src/index.ts` の `sendNotification()` 呼び出しを `sendWebPushNotification(pushSubscription, payload, vapidConfig)` に変更する（依存: T013）
- [x] T015: `services/stock-tracker/batch/src/lib/web-push-client.ts` の `sendNotification()` を削除し、ペイロード生成関数のみ残す（依存: T012）
- [x] T016: `services/stock-tracker/batch/src/minute.ts`, `hourly.ts` の `sendNotification(alert, ...)` 呼び出しを `sendWebPushNotification(alert.subscription, payload, vapidConfig)` に変更する（依存: T015）
