...
**理由**: 
- 本サービスの設計思想は「事前にDynamoDBに動画を登録し、条件指定によるフィルタリングとランダム選択を行う」というワークフローを前提としている
- 動画IDリストの一時的な即座登録を許可すると、データ管理が複雑化し、お気に入りフラグやスキップフラグなどのユーザー設定が適用できない
- 既存の「動画一括インポート」機能（UC-002）により、事前登録は容易に実現可能

**将来的な対応**: 
- ユーザーからの強い要望があれば、「一時的なマイリスト登録モード」として検討可能
- ただし、DynamoDBへの保存を前提としたワークフローを推奨する
...
**理由**: 
- ニコニコ動画特有のマイリスト機能（上限100件、既存動画の削除と新規追加など）に最適化されている
- 他のサイトはAPIやマイリスト仕様が異なり、統一的な実装が困難
- 本サービスの主目的である「ニコニコ動画のマイリスト登録自動化」に集中することで、品質と保守性を確保

**将来的な対応**: 
- 各動画サイトごとに独立したサービスとして開発する方が現実的
- 共通ライブラリ化できる部分（Playwright Helper、認証基盤など）は別途抽出可能
...
- 複数マイリストの管理には、ユーザーがどのマイリストに登録するかを選択するUIと、マイリストの一覧取得・選択ロジックが必要
- ニコニコ動画のマイリスト取得APIが複雑であり、Playwrightによる自動操作の安定性を損なう可能性がある

**将来的な対応**: 
- ユーザーからの要望が高まれば、マイリスト選択機能を追加可能
- ただし、安全性を優先し、慎重に実装する必要がある
...
**理由**: 
- 定期的な自動登録を行うと、ユーザーが意図しないタイミングでマイリストが更新される可能性がある
- ニコニコ動画のサーバーへの配慮として、ユーザーが明示的に実行するフローを採用している
- 定期実行のインフラ（EventBridge、Lambda スケジューラーなど）が必要であり、コスト増加とメンテナンス負荷が発生する

**将来的な対応**: 
- ユーザーが手動で定期的に実行する運用を推奨
- どうしても必要な場合は、ユーザーが実行スケジュールを明示的に設定する機能として検討可能
...
**理由**: 
- 本サービスの主目的は「マイリスト登録の自動化」であり、動画の視聴機能は本質的な価値ではない
- ニコニコ動画の埋め込みプレーヤーを使用する場合、利用規約やAPIの制約を確認する必要がある
- 動画の視聴はニコニコ動画の公式サイトで行うことを推奨し、本サービスからはリンクのみ提供する

**将来的な対応**: 
- ニコニコ動画の公式APIや埋め込み規約が明確になれば、動画プレビュー機能の追加を検討可能
- ただし、サムネイル表示とリンクで十分に機能するため、優先度は低い
...
**理由**: 
- 既存実装（Python版）からのデータ移行は一度限りの作業であり、恒久的な機能として実装する必要性が低い
- DynamoDB スキーマが異なる場合、移行スクリプトを個別に作成する方が効率的
- 移行は手動でエクスポート/インポートするか、AWS CLI や SDK を使用したスクリプトで対応可能

**将来的な対応**: 
- 必要に応じて、一時的な移行スクリプトを別途作成することは可能
- ただし、本サービスの機能としては組み込まない
...
**理由**: 
- 動画のダウンロードは著作権法やニコニコ動画の利用規約に抵触する可能性が高い
- 本サービスの目的は「マイリスト登録の自動化」であり、ダウンロード機能は方針外
- 法的リスクを避けるため、明確にスコープ外とする

**将来的な対応**: 
- 対応予定なし（法的・倫理的な理由から実装しない）
...
**理由**: 
- 本サービスの主目的は「マイリスト登録の自動化」であり、コメント機能は本質的な価値ではない
- ニコニコ動画のコメントAPIは公式に提供されておらず、非公式な方法での取得は安定性とセキュリティに問題がある
- コメントの閲覧はニコニコ動画の公式サイトで行うことを推奨

**将来的な対応**: 
- ニコニコ動画の公式APIでコメント取得が可能になれば検討可能
- ただし、優先度は低い
...
**理由**: 
- 現時点では Material-UI のデフォルトテーマ（ライトモード）を使用することで、開発速度を優先
- ダークモード対応には、カラーパレットの再設計とテーマ切り替え機能の実装が必要
- 本サービスの主目的である「マイリスト登録の自動化」には影響しない

**将来的な対応**: 
- Material-UI のテーマ切り替え機能を使用することで、比較的容易に実装可能
- ユーザーからの要望が高まれば対応を検討
...
**理由**: 
- 本サービスは日本のニコニコ動画を対象としており、対象ユーザーは日本語話者が中心
- 多言語対応には、翻訳リソースの作成と i18n ライブラリの導入が必要
- 現時点では日本語のみで十分にサービスの価値を提供可能

**将来的な対応**: 
- 海外ユーザーからの要望があれば、Next.js の i18n 機能を使用して多言語対応を検討可能
- ただし、ニコニコ動画自体が日本語中心のサービスであるため、優先度は低い
...
**理由**: 
- Material-UI のデフォルトテーマで十分にモダンで使いやすいUIを提供可能
- カスタムテーマの作成には、デザイン工数と実装工数が追加で必要
- 本サービスの主目的である「マイリスト登録の自動化」には影響しない

**将来的な対応**: 
- プラットフォーム全体で統一されたカスタムテーマが作成された場合、それを適用可能
- 現時点では Material-UI のデフォルトテーマを使用
...
**理由**: 
- 基本的なトランジション（Fade、Slide など）で十分に使いやすいUIを提供可能
- 詳細なアニメーション演出には、実装工数とパフォーマンスへの配慮が必要
- 本サービスはツールとしての機能性を重視し、過度な演出は避ける

**将来的な対応**: 
- ユーザー体験を向上させる必要がある場合、Material-UI の Transition コンポーネントを活用して段階的に追加可能
- ただし、パフォーマンスを損なわない範囲で実装

---

#### ❌ オフライン対応（PWA の Service Worker）

**理由**: 
- 本サービスはニコニコ動画 API や AWS Batch との通信が必須であり、オフラインでの動作は実質的に不可能
- Service Worker の実装には追加の開発工数とメンテナンス負荷が発生
- 現時点ではオンライン環境での利用を前提とする

**将来的な対応**: 
- 動画一覧のキャッシュや、オフライン時のUI表示など、部分的なオフライン対応は検討可能
- ただし、優先度は低い
...
| 用語                 | 定義                                                                                                     |
| -------------------- | -------------------------------------------------------------------------------------------------------- |
| ニコニコ動画         | 日本の動画共有サービス。本サービスは、ニコニコ動画のマイリスト機能を対象とする。                         |
| マイリスト           | ニコニコ動画におけるユーザー個別の動画コレクション機能。1つのマイリストに最大100個の動画を登録可能。      |
| 動画ID               | ニコニコ動画の動画を一意に識別する ID。`sm12345678` のような形式。                                       |
| お気に入りフラグ     | ユーザーが特定の動画を「お気に入り」としてマークするためのフラグ。登録条件として使用可能。               |
| スキップフラグ       | ユーザーが特定の動画をマイリスト登録から除外するためのフラグ。登録条件として使用可能。                   |
...
| Google User ID                       | Auth プロジェクトで使用される Google OAuth 認証によるユーザー識別子。ユーザーデータの分離に使用。        |

### 技術用語

| 用語                 | 定義                                                                                                     |
| -------------------- | -------------------------------------------------------------------------------------------------------- |
| Playwright           | Microsoft 製のブラウザ自動化ツール。本サービスでは、ニコニコ動画へのログインとマイリスト登録の自動化に使用。 |
| AWS Batch            | AWS のバッチ処理サービス。長時間実行が必要なマイリスト登録処理を実行するために使用（Lambda の15分制限を回避）。 |
| バッチ処理           | バックグラウンドで実行される長時間処理。本サービスでは、AWS Batch を使用してマイリスト一括登録を実行。     |
| Web Push             | ブラウザへのプッシュ通知機能。バッチ処理完了時にユーザーへ通知を送信するために使用。                     |
| DynamoDB             | AWS の NoSQL データベースサービス。動画基本情報とユーザー設定を格納。                                    |
| getthumbinfo         | ニコニコ動画の非公式 API エンドポイント（`https://ext.nicovideo.jp/api/getthumbinfo/{videoId}`）。動画基本情報を XML 形式で取得。 |
