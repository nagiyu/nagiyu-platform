...
- **正常フロー**:
    ...
    8. AWS Batch が各動画をマイリストに登録（**各動画間に最低 2 秒待機**）
    9. システムが完了通知を Web Push で送信
    10. ユーザーが登録結果を確認
- **代替フロー**:
    - **3a**: 条件に合致する動画が 100 個未満の場合、取得したすべての動画を登録対象とする
    - **7a**: ユーザーのマイリストが存在しない場合、システムが自動的に新しいマイリストを作成する
    - **7b**: 既存のマイリストが複数ある場合、最初のマイリストに追加する
- **例外フロー**:
...
| F-005  | 動画情報管理               | お気に入りフラグ、スキップフラグ、メモの編集・削除                                   | 中     |
| F-006  | 動画一覧表示               | DynamoDB に登録された動画を一覧表示                                                  | 中     |
| F-007  | Google OAuth 認証          | Google アカウントでログイン・ログアウト                                              | 高     |
