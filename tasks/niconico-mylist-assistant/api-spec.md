...
ロールベースのアクセス制御は使用せず、認証ユーザーは自分の UserID に紐づくデータのみアクセス可能です。
...
**実装詳細**:
- Auth プロジェクトで認証されたユーザーは、自動的に `niconico-mylist:use` 権限が付与される
...
- Auth プロジェクトで `niconico-mylist:use` 権限が付与されていない場合（通常は発生しない）
