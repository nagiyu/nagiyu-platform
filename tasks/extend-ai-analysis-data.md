...
- [ ] T012: OpenAI 呼び出しを vision 対応の messages 形式に変更する
    ...
    - `gpt-5-mini` は vision 対応のため、モデル変更は不要
    - 影響ファイル: `batch/src/lib/openai-client.ts`
...
- **モデルの選定**: `gpt-5-mini` は画像入力（vision）に対応しているため、モデルの変更は不要
