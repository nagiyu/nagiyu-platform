...
- [x] T001: `web/types/stock.ts` の `TickerSummary` に `buyAlertCount` / `sellAlertCount` フィールドを追加
  - 型: `{ enabled: number; disabled: number }`
- [x] T002: `app/api/summaries/route.ts` で `alertRepository.getByUserId()` を呼び出し、TickerID × Mode で有効・無効件数を集計してレスポンスに含める
- [x] T003: `tests/unit/app/api/summaries/route.test.ts` のテストを更新（アラート件数が含まれることを検証）

### Phase 2: フロントエンド変更

- [x] T004: `app/summaries/page.tsx` の OHLC 列（始値・高値・安値・終値）を削除
- [x] T005: `app/summaries/page.tsx` に「投資判断」列を追加（`resolveInvestmentSignalLabel()` 使用）
- [x] T006: `app/summaries/page.tsx` に「買いアラート数」「売りアラート数」列を追加
  - 表示形式: 有効数のみの場合 `1`、無効数もある場合 `1 (2)`（有効 1、無効 2）、すべて 0 の場合 `0`

### Phase 3: テスト更新・検証

- [x] T007: `tests/e2e/summary-display.spec.ts` を新しい列構成に合わせて更新
- [x] T008: ユニットテスト・E2E テストを実行して全件通過を確認
- [x] T009: スマートフォン表示でテーブルのレイアウトを確認
