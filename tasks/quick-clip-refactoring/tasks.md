...
- [x] `web/src/app/page.tsx`: `uploadProgress` state を追加し、小ファイルアップロードを `XMLHttpRequest` に変更して `upload.onprogress` で進捗を更新する（依存: なし）
- [x] `web/src/app/page.tsx`: マルチパートアップロードのチャンクループで `uploadedBytes / file.size` を追跡して `uploadProgress` を更新する（依存: なし）
- [x] `web/src/app/page.tsx`: `LinearProgress` と進捗テキストを追加する。アップロードボタンのラベルを動的に変更する（依存: 上記）
- [x] `web/src/app/page.tsx`: 制約通知（タブ閉じ禁止・24時間失効）を MUI `Alert` で追加する（依存: なし）
- [x] `web/src/app/jobs/[jobId]/page.tsx`: ポーリング間隔を `10000` → `5000` に変更する（依存: なし）
- [x] `web/src/app/jobs/[jobId]/page.tsx`: MUI `Stepper`（アップロード → 解析 → 切り出し）を追加する。`batchStage` のサブラベル表示を実装する（依存: Phase 3 の型変更）
- [x] `web/src/app/jobs/[jobId]/page.tsx`: 有効期限表示（`expiresAt` から日時文字列を生成）を追加する（依存: Phase 3）
- [x] `web/src/app/jobs/[jobId]/page.tsx`: タブ閉じOK通知（PENDING / PROCESSING 時）を追加する（依存: なし）
- [x] `web/src/app/jobs/[jobId]/highlights/page.tsx`: 「X / Y 件生成済」クリップ生成進捗を追加する（依存: なし）
- [x] `web/src/app/jobs/[jobId]/highlights/page.tsx`: 有効期限を表示するために初回マウント時に `GET /api/jobs/[jobId]` を呼んで `expiresAt` を取得する（依存: Phase 3）
- [x] フロントエンドのユニットテストを更新・追加する（依存: 上記全て）
