...
- [x] T003: `.github/workflows/templates/daily-refactoring-body.md` を作成
...
- [x] T004: `.github/workflows/scripts/prepare-refactoring-issue.sh` を作成
  - 環境変数: `SERVICE_LIST`, `NEXT_DATE`, `CREATE_TIME`
  - テンプレートファイルを読み込み、変数置換して本文を標準出力
  - ※重複チェックのスキップ処理はワークフロー側で完結させ、スクリプトは Issue 本文生成のみを担う
- [x] T005: `.github/workflows/daily-refactoring-check.yml` を作成
