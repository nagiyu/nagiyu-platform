...
| 要素 | 種別 | 説明 |
|-----|------|------|
| 動画プレビュー | 動画プレイヤー | 画面上部に配置。選択中の見どころの区間を再生する |
| 見どころ一覧テーブル | テーブル | プレビュー下部に配置。番号・開始〜終了時刻・採否・時間調整を行単位で表示 |
...
| 操作 | 結果 |
|------|------|
| 見どころ行をクリック | 上部の動画プレビューが該当する動画区間に切り替わり再生される |
...
- 採用0件: ダウンロードボタンを無効化
- PoC 補足: 動画プレビューは UI 枠のみ先行実装し、実動画の再生連携は Phase 5 以降で実装する
...
- 動画の変化量（フレーム差分）と音量の大きさそれぞれで上位10件を抽出し、計20件を見どころとして表示する
