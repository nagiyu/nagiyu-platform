...
- [x] T001: `libs/nextjs` に `createAuthMiddleware()` ファクトリ関数を実装する
...
- [x] T002: 各サービスの `middleware.ts` を `createAuthMiddleware()` で置き換える
...
- [x] T005: `libs/common` に共通エラーメッセージ定数を追加する
...
- [x] T006: 各サービスのエラーメッセージ定数を `COMMON_ERROR_MESSAGES` をスプレッドする形に変更する
...
- [x] T007: `libs/aws` に Batch クライアントのファクトリを追加する
  - S3・DynamoDB と同様のシングルトンパターンで実装する
  - 対象ファイル: `libs/aws/src/batch/index.ts`（新規）、`libs/aws/src/index.ts`
- [x] T008: `libs/aws` 未使用サービスを `libs/aws` 経由に移行する
  - 対象: `services/share-together/web/src/lib/aws-clients.ts`（DynamoDB クライアント取得を `@nagiyu/aws` 経由に移行）
  - 対象: `services/codec-converter/web/src/lib/aws-clients.ts`（S3・Batch。T007 完了後に着手。DynamoDB は移行済み）
  - 補足: T000 記載時点との差分を確認し、share-together は DynamoDB の移行対象として更新

### フェーズ 3: 構成統一・既存パターンの普及（優先度：低）

- [x] T009: 手動実装のヘルスチェックルートを `createHealthRoute()` に統一する
  - 対象: `services/share-together/web/src/app/api/health/route.ts`（唯一の手動実装サービス）
- [x] T010: 各サービスの `next.config.ts` の `transpilePackages` を標準化する
  - 全サービスで参照すべきパッケージリストをドキュメント化し、各サービスの設定を統一する
  - 対象: 全サービスの `next.config.ts`
  - 標準リスト（ベース）: `@nagiyu/ui`, `@nagiyu/browser`, `@nagiyu/common`, `@nagiyu/nextjs`
  - 追加リスト（サービス固有）: `@nagiyu/auth-core`, `@nagiyu/aws`, `@nagiyu/react`, `@nagiyu/*-core`, `codec-converter-core`
