...
-   [x] T006: 各サービスのエラーメッセージ定数を `COMMON_ERROR_MESSAGES` をスプレッドする形に変更する
    ...
    -   追記（2026-03-12）: `services/niconico-mylist-assistant/web/src/components/JobStatusDisplay.tsx` の 2FA 正規表現参照は、ジョブ確認画面のクライアント例外回避のため core パッケージ実行時 import ではなくローカル定数へ修正
