...
- [x] `SEGMENTS_PER_CHUNK = 50` 定数を追加する（`OPENAI_MODEL` などの既存定数と同じ箇所に追記）
- [x] `chunkArray<T>(array: T[], size: number): T[][]` ヘルパー関数を追加する（ファイル内部のみ、export 不要）
- [x] `EmotionHighlightService.getScores` をチャンク処理に変更する（design.md の「変更後実装」を参照）

## Phase 2: テストの追加

<!-- 既存テスト `core/tests/unit/libs/emotion-highlight.service.test.ts` にケースを追加する -->

- [x] テストデータ生成ヘルパー `makeSegments(count)` を追加する（design.md のヒントを参照）
- [x] 正常系（チャンク境界: 50件）: `mockParse` が 1 回呼ばれることを確認
- [x] 正常系（チャンク境界: 51件）: `mockParse` が 2 回呼ばれ、結果が結合されることを確認
- [x] 異常系（チャンク内リトライ）: 1チャンク目が 1 回失敗後に成功するケースを確認
...
- [x] `requirements.md` の受け入れ条件をすべて満たしている
  - [x] 50 件以下のセグメントでは API が 1 回だけ呼ばれる
  - [x] 51 件のセグメントでは API が 2 回呼ばれ、結果が結合されて返る
  - [x] チャンク単位でリトライが動作する
  - [x] 既存のすべての単体テストが引き続きパスする
- [x] Lint・型チェックがすべて通過している
- [x] `design.md` の「docs/ への移行メモ」を処理した
- [x] `docs/services/quick-clip/architecture.md` を更新した
