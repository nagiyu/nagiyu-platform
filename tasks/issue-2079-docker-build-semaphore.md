...
共有インフラは `infra/shared/` 配下で管理されており、VPC・ACM・IAM を別スタックで定義している。今回追加するロック用 S3 バケットも同様に独立したスタックとして追加するか、既存の Application スタックに統合するかを検討する。
...
スタック設計の考慮点:
- バケット名は環境非依存にする（ロックはどの環境のビルドでも共有するため）または環境ごとに作成する
- バケットの公開アクセスはブロックする
- バケット名を SSM パラメータまたは環境変数で各ワークフローから参照できるようにする
- ライフサイクルルールで古いロックオブジェクト（例: 1日以上経過したもの）を自動削除する（ビルド失敗時のゾンビロック対策）
...
- [ ] T001: `infra/shared/lib/docker-build-lock-stack.ts` を新規作成する
- [ ] T002: ライフサイクルルールを追加してゾンビロック対策を実装する
- [ ] T003: `infra/shared/bin/shared.ts` に新スタックを登録する
- [ ] T004: `infra/shared/` の CDK テストを更新・追加する
- [ ] T005: 既存 IAM ポリシーがロック用バケットに対して必要な権限を付与していることを確認する（不足があれば `iam-application-policy-stack.ts` を更新する）
...
- [ ] T007: ロック取得処理を実装する
- [ ] T008: ロック解放処理を実装する
- [ ] T009: S3 バケット名を取得する方法を確立する（例: 環境変数 `DOCKER_BUILD_LOCK_BUCKET` をシークレットまたは CDK Outputs/SSM 経由で渡す）
...
- S3 バケットを環境ごとに作成するか（dev/prod）、1 つの共有バケットにするかは要検討
    - ロックの目的はレートリミット対策であり、dev/prod 間でも並列実行が問題になりうるため、共有バケット（1 つ）が望ましい可能性がある
- ロック上限数（3）は issue の指示に従い固定値とする
- ロック取得のポーリング間隔（30 秒）は issue の指示に従い固定値とする
- 上限超過の許容については NFR1 のとおり、完全な排他制御は不要
- ゾンビロックのライフサイクル期間（例: 1 日）は実装時に検討する
- `infra/shared/` に CDK テストが存在するかは実装前に確認が必要
