...
- [x] T007: `web/public/sw.js` の `notificationclick` ハンドラーが `data.url` を使用することを確認（既存実装のまま動作するはず）
- [x] T008: `web/components/HomePageClient.tsx` に `useSearchParams` によるクエリパラメータ読み取り・初期選択を追加
- [x] T009: `web/components/AlertSettingsModal.tsx` にカスタム通知タイトル・本文の入力フィールドを追加
- [x] T010: `web/app/api/alerts/route.ts` および `[id]/route.ts` のリクエスト型を更新
- [x] T011: `web` のユニットテストおよび E2E テストを更新・追加
