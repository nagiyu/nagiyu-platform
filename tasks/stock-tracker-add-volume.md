...
- [ ] T008: `batch/src/lib/openai-client.ts` の `HistoricalPriceData` に `volume?: number` を追加（始値・終値と同等の扱い）
- [ ] T009: `createPrompt` 関数の当日データセクション（始値・終値・高値・低値のセクション）に出来高を追加
- [ ] T010: `createPrompt` 関数の過去データセクションにも出来高を追加
- [ ] T011: `batch/src/summary.ts` にて最新日足データから `volume` を抽出し `upsert` 入力へ連携
- [ ] T012: `batch/src/summary.ts` にて `historicalData` の各レコードにも `volume` を含める
- [ ] T013: 関連するユニットテストを更新・追加

### Phase 3: Web フロントエンドの更新

- [ ] T014: `web/types/stock.ts` の `TickerSummary` に `volume?: number` を追加
- [ ] T015: `web/app/api/summaries/route.ts` のレスポンスマッピングで `Volume` を含める
- [ ] T016: サマリー詳細画面（`web/app/summaries/page.tsx`）に出来高表示UI を追加（値なしの場合は「-」表示）
- [ ] T017: 関連するユニットテスト・E2E テストを更新・追加
