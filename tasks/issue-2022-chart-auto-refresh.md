...
-   NFR1: 自動更新のインターバルは定数（`AUTO_REFRESH_INTERVAL_MS`）で管理する
...
-   `services/stock-tracker/web/components/StockChart.tsx`
    -   `useInterval` または `useEffect` + `setInterval` による定期更新ロジックを追加
...
-   `StockChart` に `autoRefresh: boolean` プロパティを追加し、コンポーネント内部でタイマーを管理する方針が自然
...
-   `HomePageClient` の既存のセレクターグループ（グリッドレイアウト）に追加するか、チャートエリアの上部に配置する

## タスク

-   [ ] T001: `StockChart` に `autoRefresh?: boolean` プロパティを追加し、`useEffect` でタイマー管理を実装する
-   [ ] T002: `HomePageClient` に `autoRefresh` 状態を追加し、トグルボタンを実装する
-   [ ] T003: `AUTO_REFRESH_INTERVAL_MS` 定数を `types/stock.ts` に追加する
-   [ ] T004: `StockChart` のユニットテストを更新・追加する（自動更新の開始・停止・クリーンアップ）
-   [ ] T005: E2E テストを追加する（自動更新ボタンの表示・動作確認）
-   [ ] T006: テストカバレッジが 80% 以上であることを確認する
...
-   自動更新ボタンの配置場所（セレクターグループ内 vs チャートエリア上部）は実装時に確認が必要
