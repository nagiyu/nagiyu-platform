...
5. [ ] 既存の手動入力モードが引き続き正常に動作する
6. [ ] 範囲指定モードが引き続き正常に動作する
7. [ ] ウォッチリストからアラート設定を開いた場合、基準価格がないためパーセンテージ選択が適切に無効化される
8. [ ] テストカバレッジが80%以上
9. [ ] E2Eテストが全てのデバイス（chromium-mobile含む）でパス
10. [ ] TypeScript strict mode でコンパイルエラーなし
11. [ ] ESLintとPrettierのチェックがパス
12. [ ] エラーメッセージが日本語で `ERROR_MESSAGES` に定数化されている
