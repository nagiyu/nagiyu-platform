...
以下の 5 本を全て追加する。いずれもブラウザ内で完結し、サーバーへのデータ送信不要:

1. **VAPID キー生成ツール**（`vapid-generator`）
    - Web Push 通知実装時に必要な公開鍵/秘密鍵ペアを生成
    - Web Crypto API（`crypto.subtle.generateKey` / P-256 曲線）で実装
    - 公開鍵・秘密鍵を Base64url 形式で出力
2. **Base64 エンコーダー/デコーダー**（`base64`）
    - テキスト・バイナリデータの Base64 変換
    - `btoa` / `atob` + TextEncoder/TextDecoder で実装
3. **URL エンコーダー/デコーダー**（`url-encoder`）
    - パーセントエンコーディングの変換
    - `encodeURIComponent` / `decodeURIComponent` で実装
4. **ハッシュ生成ツール**（`hash-generator`）
    - SHA-256 / SHA-512 のハッシュ値を生成
    - Web Crypto API（`crypto.subtle.digest`）で実装（MD5 は Web Crypto API 非対応のため対象外）
...
- [ ] T009-1: `lib/vapid.ts` に Web Crypto API（P-256）を使ったキー生成ロジックを実装
- [ ] T009-2: `app/vapid-generator/page.tsx` に UI 実装（公開鍵/秘密鍵をそれぞれコピー可能な形で表示）
- [ ] T009-3: ホームページのツール一覧に追加

#### T010: Base64 エンコーダー/デコーダー（`base64`）

- [ ] T010-1: `lib/base64.ts` にエンコード/デコードロジックを実装（`btoa`/`atob` + UTF-8 対応）
- [ ] T010-2: `app/base64/page.tsx` に UI 実装（入力/出力エリア、エンコード/デコード切り替え）
- [ ] T010-3: ホームページのツール一覧に追加

#### T011: URL エンコーダー/デコーダー（`url-encoder`）

- [ ] T011-1: `lib/url-encoder.ts` に `encodeURIComponent`/`decodeURIComponent` ラッパーを実装
- [ ] T011-2: `app/url-encoder/page.tsx` に UI 実装（入力/出力エリア、エンコード/デコード切り替え）
- [ ] T011-3: ホームページのツール一覧に追加

#### T012: ハッシュ生成ツール（`hash-generator`）

- [ ] T012-1: `lib/hash.ts` に Web Crypto API（`crypto.subtle.digest`）を使った SHA-256/SHA-512 実装
- [ ] T012-2: `app/hash-generator/page.tsx` に UI 実装（アルゴリズム選択、Hex/Base64 出力切り替え）
- [ ] T012-3: ホームページのツール一覧に追加

#### T013: タイムスタンプ変換ツール（`timestamp-converter`）

- [ ] T013-1: `lib/timestamp.ts` に Unix タイムスタンプ（秒/ミリ秒）と日時文字列の相互変換ロジックを実装
- [ ] T013-2: `app/timestamp-converter/page.tsx` に UI 実装（秒/ミリ秒の入力、フォーマット・タイムゾーン表示）
- [ ] T013-3: ホームページのツール一覧に追加

#### T014: about ページの一括更新

- [ ] T014-1: `about` ページの「提供ツール」セクションに全新規ツールを追加
- [ ] T014-2: `about` ページの「今後の展望」を新規ツール追加後の状態に更新（または削除を検討）

#### T015: schema.org JSON-LD（新規ツール分）

- [ ] T015-1: 全新規ツールページに `WebApplication` schema.org JSON-LD を追加
...
1. **タイムスタンプ変換ツールの仕様**
    ...
    - 対応する日時フォーマット（ISO 8601、RFC 2822、カスタム等）

2. **ハッシュ生成ツールの出力形式**
    - Hex のみ vs Hex / Base64 切り替え対応
    - ファイルハッシュ（ファイルアップロード対応）の要否

3. **about ページの「今後の展望」セクションの扱い**
    - 全ツールが実装済みになった後、セクションを削除するか、将来のツール追加計画に置き換えるか
    - T014-2 の実装時に確認する
