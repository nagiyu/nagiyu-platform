...
- [ ] T009-1: `app/api/vapid/route.ts` に Node.js `crypto` モジュール（`generateKeyPair`）を使ったキー生成 API を実装
- [ ] T009-2: `lib/vapid.ts` に API 呼び出しロジックを実装
- [ ] T009-3: `app/vapid-generator/page.tsx` に UI 実装（公開鍵/秘密鍵をそれぞれコピー可能な形で表示）
- [ ] T009-4: ホームページのツール一覧に追加

#### T010: Base64 エンコーダー/デコーダー（`base64`）

- [ ] T010-1: `lib/base64.ts` にエンコード/デコードロジックを実装（クライアントサイドまたは API Route 経由）
- [ ] T010-2: `app/base64/page.tsx` に UI 実装（入力/出力エリア、エンコード/デコード切り替え）
- [ ] T010-3: ホームページのツール一覧に追加

#### T011: URL エンコーダー/デコーダー（`url-encoder`）

- [ ] T011-1: `lib/url-encoder.ts` に変換ロジックを実装（クライアントサイドまたは API Route 経由）
- [ ] T011-2: `app/url-encoder/page.tsx` に UI 実装（入力/出力エリア、エンコード/デコード切り替え）
- [ ] T011-3: ホームページのツール一覧に追加

#### T012: ハッシュ生成ツール（`hash-generator`）

- [ ] T012-1: `lib/hash.ts` に SHA-256/SHA-512 の Hex 出力ロジックを実装（クライアントサイドまたは API Route 経由）
- [ ] T012-2: `app/hash-generator/page.tsx` に UI 実装（アルゴリズム選択、Hex 形式で出力）
- [ ] T012-3: ホームページのツール一覧に追加

#### T013: タイムスタンプ変換ツール（`timestamp-converter`）

- [ ] T013-1: `lib/timestamp.ts` に Unix タイムスタンプ（秒/ミリ秒）と日時文字列の相互変換ロジックを実装（Stock Tracker の実装を参考に、`@nagiyu/stock-tracker-*` の Import は禁止・コード重複は許容）
- [ ] T013-2: `app/timestamp-converter/page.tsx` に UI 実装（秒/ミリ秒の入力と日時への変換、Stock Tracker でサポート済みのフォーマット・タイムゾーンに準拠）
- [ ] T013-3: ホームページのツール一覧に追加

#### T014: about ページの一括更新

- [ ] T014-1: `about` ページの「提供ツール」セクションに全新規ツールを追加
- [ ] T014-2: `about` ページの「今後の展望」セクションを削除
