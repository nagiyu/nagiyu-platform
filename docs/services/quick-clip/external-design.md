...
| 要素 | 種別 | 説明 |
|-----|------|------|
| 制約通知 | Alert（info） | 常時表示。「アップロード中はタブを閉じないでください」「データは 24 時間で自動削除されます」を表示する |
| ファイル選択エリア | ドロップゾーン | ドラッグ&ドロップまたはクリックでファイルを選択 |
| アップロードボタン | ボタン | ファイル選択後に有効化される。アップロード中はラベルを「アップロード中... X%」→「処理開始中...」と動的に更新する |
| 進捗バー | LinearProgress | アップロード中のみ表示。0〜100% で進捗を表示する |
...
- アップロード進捗: アップロード中はプログレスバーで進捗（0〜100%）を表示する
...
| 有効期限表示 | テキスト | 常時表示。「データの有効期限: ◯◯◯◯/◯◯/◯◯ ◯◯:◯◯」（`expiresAt` から算出） |
...
| 文字起こし表示 | テキスト | 左パネルに配置。選択中クリップの時間範囲に重なる文字起こしテキストを表示する。該当テキストが無い場合は非表示 |
...
| クリップ生成進捗 | テキスト + CircularProgress | 右パネルのテーブルヘッダー付近に配置。クリップ生成中（PENDING または GENERATING が 1 件以上存在する間）のみ表示。「X / Y 件生成済」形式で表示する |
| 有効期限表示 | テキスト | ページ上部（テーブルヘッダー付近）に常時表示。「データの有効期限: ◯◯◯◯/◯◯/◯◯ ◯◯:◯◯」（`job.expiresAt` から算出） |
...
- ダウンロード ZIP の内容: 採用クリップを `clip-NN.mp4`（No. 連動・ゼロ埋め2桁）として格納し、文字起こしテキストがあるクリップには同名の `clip-NN.txt` を同梱する
