...
専用テーブル `nagiyu-error-events-{env}` を `infra/shared/lib/error-events-table-stack.ts` で新設する。Admin は読み取り側、各サービスは書き込み側として共有利用する。
...
| 属性 | 用途 |
| --- | --- |
| `PK = ERROR_EVENT#{serviceId}` / `SK = OCCURRED#{occurredAt}#{eventId}` | サービス指定の時系列クエリ |
| `GSI1PK = ERROR_EVENT_ALL` / `GSI1SK = OCCURRED#{occurredAt}#{eventId}` | 全サービス横断の時系列クエリ |
| `ttl`（unix epoch 秒） | 180 日後に自動削除 |
...
- BatchSize: 100、MaxBatchingWindow: 5 秒、RetryAttempts: 2、`bisectBatchOnError: true`
- 失敗イベントは SQS DLQ（`nagiyu-admin-stream-handler-dlq-{env}`、保持 14 日）
...
- Topic が 2 つに増える
- 自己監視のアラーム定義は `infra/admin/lib/self-monitoring-alarms-stack.ts` に集約することで運用負荷を抑える

**監視対象（最小セット）**:

- `alarm-ingest` Lambda の Errors（5 分間 1 件以上）
- `stream-handler` Lambda の Errors（同上）
- `stream-handler` DLQ の `ApproximateNumberOfMessagesVisible`（1 件以上）
- `error-events` テーブルの SystemErrors / ThrottledRequests（同上）
