...
- **変換パラメータ**:
    ...
    - 音声コーデック: 各コンテナに最適なコーデックで再エンコード（H.264=AAC, VP9/AV1=Opus）
...
- **FFmpegエラー**:
    ...
    - ステータスを `FAILED` に設定
- **リトライ戦略**:
    - 一時的なエラー（ネットワーク障害、S3アクセスエラーなど）は1回リトライ（計2回試行）
    - リトライ間隔: AWS Batch のデフォルト動作に従う
    - 1回リトライ後も失敗した場合は `FAILED` に設定
- **中間ファイルの扱い**:
...
- **処理キュー**: AWS Batch（デフォルトスケジューリング、最大3ジョブ並列実行）
...
ジョブの特性に応じて最適なリソースを動的に割り当てることで、コスト効率と処理時間を最適化します。

**実装状況**: 2026年2月に実装完了

**詳細**: [architecture.md の動的リソース配分セクション](./architecture.md#動的リソース配分) を参照

#### リソース選択の自動化

- **要件**: ファイルサイズと出力コーデックに基づいて、最適な vCPU とメモリを自動選択
- **目的**:
    - 軽量な処理には低スペックリソースを使用してコストを削減
    - 重量な処理には高スペックリソースを使用して処理時間を短縮
    - 同時実行可能なジョブ数を最大化してキュー滞留を削減

#### コスト最適化

- **要件**: リソース構成の異なる複数のジョブ定義を用意し、適切に使い分ける
- **目標**:
    - 小さいファイル（< 100MB）+ H.264 の場合、1 vCPU / 2048 MB で処理
    - 大きいファイル（> 300MB）+ AV1 の場合、4 vCPU / 16384 MB で処理
    - 中間的なケースは適切なリソースを選択
- **効果測定**: AWS Cost Explorer でリソース最適化前後のコストを比較

#### 処理時間の最適化

- **要件**: 重量な処理には高性能リソースを割り当てることで処理時間を短縮
- **目標**:
    - AV1 変換の処理時間を 20-30% 短縮（4 vCPU 使用時）
    - VP9 変換の処理時間を 10-20% 短縮（4 vCPU 使用時）
- **測定方法**: CloudWatch Logs でジョブ実行時間を記録・比較

#### キャパシティ管理

- **要件**: Compute Environment の maxvCPUs を拡張し、多様なリソース構成のジョブを同時実行可能にする
- **現状**: maxvCPUs = 6（2 vCPU × 3 ジョブ）
- **Phase 2**: maxvCPUs = 16（軽量ジョブを多数並列実行、または重量ジョブを少数実行）
- **例**: 1 vCPU ×4 + 4 vCPU ×2 = 12 vCPU（合計6ジョブ同時実行可能）

#### 後方互換性

- **要件**: 既存のジョブが失敗しないように、デフォルトのジョブ定義（medium: 2 vCPU / 4096 MB）を引き続き利用可能にする
- **実装方針**:
    - リソース選択ロジックが失敗した場合、デフォルトのジョブ定義にフォールバック
    - 不正なファイルサイズ・コーデック指定時も medium を使用

#### モニタリング

- **要件**: 各リソースパターンの使用状況を CloudWatch Logs で確認可能にする
- **実装**:
    - ジョブ定義名をログに出力
    - 使用リソース（vCPU、メモリ）を起動時に記録
    - リソース選択の判定理由をログに記録

#### テストカバレッジ

- **要件**: リソース決定ロジックのユニットテストでカバレッジ 80% 以上を達成
- **テスト項目**:
    - 各ファイルサイズ × コーデックの組み合わせで正しいジョブ定義が選択されるか
    - 境界値テスト（100MB、300MB）
    - 不正な入力のエラーハンドリング
    - デフォルトへのフォールバック動作
