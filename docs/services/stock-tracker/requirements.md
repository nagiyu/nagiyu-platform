...
**概要**: ユーザーが各取引所に登録されたティッカーの直近の日次サマリー（始値・高値・安値・終値）を一覧形式で確認する
...
2. システムが各取引所の最新保存済みサマリーを取得する（当日分が未生成の場合は直近保存済み日付のデータを表示し、空表示にしない）
3. 取引所ごとにグループ化されたティッカー一覧と、各ティッカーの銘柄名・シンボル・始値・高値・安値・終値が表示される
...
| F-011  | 日次サマリー表示           | 取引所ごとのティッカー OHLC 一覧を表示（`stocks:read` 権限） | 高     |
...
| サマリー一覧画面           | `/summaries`  | 取引所ごとのティッカー日次 OHLC 一覧表示（`stocks:read` 権限） | 高     |
