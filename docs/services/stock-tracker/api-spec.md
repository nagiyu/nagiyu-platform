            ...
            "summaries": [
                {
                    ...
                    "aiAnalysisResult": {
                        ...
                        "patternAnalysis": "パターン分析の解釈",
                        "supportLevels": [182.0, 180.5, 179.2],
                        "resistanceLevels": [185.0, 186.4, 188.0],
                        "relatedMarketTrend": "関連市場・セクター動向",
                        "investmentJudgment": {
                            "signal": "NEUTRAL",
                            "reason": "短期は方向感が限定的"
                        }
...
- `aiAnalysisResult` は AI 解析生成成功時のみ値を持つ（未生成・失敗時は `null` 相当）
...
- Web Push 通知
- 日次サマリー表示（OHLC + 日足パターン分析結果、出来高は対象外）
