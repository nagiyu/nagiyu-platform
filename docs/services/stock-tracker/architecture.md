...
- 初版は固定パターン（買い1種・売り1種）を採用し、ユーザーによる定義変更は行わない
- 判定結果は **成立 / 不成立 / 判定不能** の3状態で扱い、判定不能を通常の不成立と分離する
- 取得日足が所定本数に満たない場合は、成立判定を行わず判定不能として保持する
- 一覧画面では成立件数のみを表示し、詳細画面で内訳と説明を確認できる構成とする
...
- `USE_IN_MEMORY_DB=true`: インメモリリポジトリを使用（E2E テスト時）
- `USE_IN_MEMORY_DB` 未設定または `false`: DynamoDB リポジトリを使用（本番環境）
...
| エンティティ     | PK              | SK                | GSI1                                     | GSI2                                   | GSI3                                    | GSI4                                         | 説明                        |
| ---------------- | --------------- | ----------------- | ---------------------------------------- | -------------------------------------- | --------------------------------------- | -------------------------------------------- | --------------------------- |
...
4. **GSI4 (ExchangeSummaryIndex) - 取引所別サマリークエリ**:
   - **役割**: 取引所別の日次サマリー一覧を取得
   - **GSI4PK**: `{ExchangeID}` - 取引所IDでパーティション
   - **GSI4SK**: `DATE#{Date}#{TickerID}` - 日付・ティッカー順でソート
   - **ユースケース**: サマリー一覧画面での取引所ごとのサマリー取得、特定日のサマリー絞り込み
   - **設計意図**: `GSI4SK begins_with DATE#{Date}` により特定日のサマリーを効率的に取得。Scan + フィルタでは大量データ時のコスト・レイテンシが許容範囲外になるため GSI を採用
...
4. **透過的内部ページネーション（取引所別サマリー取得）**:
   - GSI4 を使用した取引所別サマリー取得（`getByExchange`）では、DynamoDB が 1 回のレスポンスで返せるデータ（最大 1MB）を超えても全件を取得するため、リポジトリ内部でループを実装する
   - `LastEvaluatedKey` が返される限りクエリを繰り返し、全ページのデータを結合してから呼び出し側へ返す
   - クライアントには完全なデータセットが返り、ページネーションの詳細は隠蔽される
   - 取引所のサマリー件数が大幅に増加した場合は、レスポンス遅延が生じる可能性があるため、将来的には UI 側のページネーション対応を検討する
...
- `stock-tracker-batch-summary-{env}`: 1時間間隔の日次サマリー生成（アラート処理バッチとは独立）
...
- `stock-tracker-batch-summary-{env}`: rate(1 hour)（日次サマリー生成専用、hourly とは独立した別ルール）
...
- 通知クリック時はトップ画面（チャート画面 `/`）へ遷移する
- URL クエリパラメータ `exchangeId` / `tickerId` に通知元アラートの取引所・ティッカーを付与し、チャート画面でそれらが選択済みの状態で表示する

**カスタム通知内容**:

- アラートにカスタム通知タイトル・本文が設定されている場合は、その値を通知に使用する
- 未設定の場合はアラート情報（ティッカー・条件など）をもとに自動生成した内容を使用する
