...
**`getChartData(tickerId, 'D', { count: 50, session: 'extended' })` の **1回呼び出し**で OHLC 最新値とパターン分析用データを兼用する。**

- 既存の summary バッチの `count: 1` 呼び出しを `count: 50` に変更し、1ティッカーあたり1回の API 呼び出しに統一する
- 取得した 50 本のうち `chartData[0]` を OHLC 保存に、全 50 本を `PatternAnalyzer.analyze()` に渡す
- 結果を `DailySummary.upsert()` で統合保存する

### 根拠

- **TradingView API のレートリミット対策**: ティッカー数が数十〜数百規模になると、ティッカーごとに2回呼び出す方が1回呼び出しよりも総 API 呼び出し数が2倍になり、レートリミットに当たりやすくなる
- 1回の呼び出しで50本取得しても、50本分のデータ転送量の増加は軽微であり、呼び出し回数削減のメリットが上回る
- コードが簡潔になる（1呼び出し → 2用途に使用）

### 検討した代替案

| 代替案 | 却下理由 |
|--------|---------|
| OHLC取得 (count:1) とパターン取得 (count:50) を別々に呼び出す | ティッカーごとに2回 API 呼び出しが発生し、総呼び出し数が2倍になる。レートリミットのリスクが高まり、バッチ実行時間も増加する |
| DailySummary の過去履歴を DB から取得 | DailySummary テーブルに格納済みのデータを使えば TradingView 呼び出しを追加しないで済む利点はあるが、データ鮮度と実装複雑性のトレードオフがある。今回は TradingView から直接取得するシンプルなアプローチを採用 |
