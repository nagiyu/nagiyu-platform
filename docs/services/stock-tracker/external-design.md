...
| 日次サマリーパネル | 情報パネル | 選択銘柄の直近サマリーと投資判断を表示する。投資判断（強気/中立/弱気を色分け表示）・予測リターン・確信度・買いシグナル・売りシグナル・AI 判定・更新時間・サポートレベル・レジスタンスレベルを表示し、詳細ダイアログボタンを提供する |
...
取引所ごとにグループ化されたティッカーの日次サマリー一覧を表示する画面。各ティッカーの投資判断・予測リターン・確信度・パターン分析件数・アラート数を一覧で確認できる。`stocks:read` 権限が必要。
...
| ティッカー一覧 | リスト | シンボル・銘柄名・保有可否・投資判断（強気/中立/弱気を色分け表示）・予測リターン・確信度・パターン数・アラート数を表示する |
...
| 要素 | 種別 | 説明 |
|------|------|------|
| 主要指標テキスト | サブタイトルテキスト | 期間ラベル + 方向精度 + 採点件数 + NEUTRAL 比率 + 総合精度 の 1 行。判定済み 0 件のときは「期間ラベル: 採点済みの予測がありません」。読み込み中は「期間ラベル: 集計中...」 |
...
| シグナル別精度 | 棒グラフ（精度 + ベースライン）+ 数値テーブル | BULLISH / NEUTRAL / BEARISH ごとのヒット率・件数・ベースライン・エッジを表示する。エッジは符号付きで精度とベースラインの差分を示す |
