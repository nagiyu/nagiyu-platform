...
- [ ] `LOG_LEVEL=DEBUG` の場合、すべてのレベルが出力される
- [ ] `LOG_LEVEL=INFO` の場合、INFO, WARN, ERROR が出力される
- [ ] `LOG_LEVEL=WARN` の場合、WARN, ERROR のみが出力される
- [ ] `LOG_LEVEL=ERROR` の場合、ERROR のみが出力される
- [ ] 無効な `LOG_LEVEL` の場合、デフォルト (INFO) が使用される
- [ ] `LOG_LEVEL` が未設定の場合、デフォルト (INFO) が使用される
...
- [ ] ログレベルが正しく出力される
- [ ] メッセージが正しく出力される
- [ ] コンテキスト情報がない場合、context フィールドが省略される
- [ ] コンテキスト情報がある場合、正しくJSON化される

#### 3. JSON形式の検証

- [ ] 出力されるログが正しいJSON形式である
- [ ] JSON.parse() でパース可能である
- [ ] スキーマに準拠している

#### 4. コンテキスト情報

- [ ] 単純な値（文字列、数値）が正しく記録される
- [ ] ネストされたオブジェクトが正しく記録される
- [ ] undefined や null が適切に処理される
- [ ] 循環参照がある場合にエラーが発生しない（またはエラーハンドリング）

#### 5. エラーハンドリング

- [ ] Error オブジェクトのメッセージとスタックトレースが記録される
- [ ] カスタムエラーオブジェクトが適切に処理される
- [ ] ログ出力自体が失敗した場合でもアプリケーションがクラッシュしない

#### 6. 出力先

- [ ] INFO, DEBUG は stdout に出力される
- [ ] WARN, ERROR は stderr に出力される
