...
- **Portal の技術記事は Markdown だが `develop` 直行ではなく `integration` 経由にする**。原稿の差分では良し悪しが判断できず、dev 環境の描画を見て初めて検証できる「デプロイして確認するコンテンツ」だからである（軽量＝ develop 直、の例外）。書き方は [`portal-article`](../.claude/skills/portal-article/SKILL.md) スキルに従う
