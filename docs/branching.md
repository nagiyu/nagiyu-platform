...
- 各開発者が作業を行うためのブランチ
- `feature/{issue-number}-{slug}` 形式で作成する
- `{issue-number}` には GitHub Issue 番号のみを入れる
- `{slug}` には Issue タイトルや作業内容を英小文字・数字・ハイフンで短く要約した語を入れる
- **開発環境へのデプロイは行わない**
...
1. Issue #2835 の対応
    - `feature/2835-agents-md` を作成し作業
2. 対応がまとまり次第
    - `integration/<App名>` または `develop` にマージ
        → 開発環境へ自動デプロイ
3. 機能一式が揃ったら
    - `develop` にマージ
...
| 種類 | 命名規則・ブランチ例 | デプロイ | 目的 |
| --- | --- | --- | --- |
| 作業ブランチ | `feature/{issue-number}-{slug}` 例: `feature/2835-agents-md` | ❌ | 個別作業・WIP |
| App 統合 | `integration/<App名>` | ✅ | App 単位の統合・検証 |
| 全体統合 | `develop` | ✅ | 全サービス統合 |
| 本番 | `master` | 本番デプロイ | 安定版 |

### 作業ブランチの slug ガイドライン

- Issue タイトルや作業内容から、目的が分かる最小限の語を選ぶ
- 英小文字・数字・ハイフンのみを使う
- 日本語タイトルは意味を保った短い英語または既存のファイル名・機能名に置き換える
- 冗長な接続詞や助詞は省き、ブランチ一覧で判別できる長さにする
