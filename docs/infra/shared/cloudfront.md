...
詳細は [Route53 ドキュメント](./route53.md) を参照。新サービスのサブドメインを追加する場合は共有インフラの Route53 レコード管理スタックにレコード定義を追加します。

### Apex ドメインの扱い

apex (`example.com`) は DNS の仕様上 CNAME を貼れませんが、Route53 の **ALIAS レコード**を使えば CloudFront を直接ターゲットにできます。本プラットフォームでは Route53 レコード管理スタックで apex の ALIAS を定義しています。

> **将来**: 現状サブドメインは CNAME ですが、各サービスの CloudFront スタックが ALIAS を自動生成するように改修予定（[#2919](https://github.com/nagiyu/nagiyu-platform/issues/2919)）。完了後は新サービス追加時の手動レコード登録も不要になります。
