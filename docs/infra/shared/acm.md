...
本プラットフォームでは、ドメインの取得と権威 DNS を分離して管理しています。

- **ドメインレジストラ**: 外部レンタルサーバー（XServer）で取得・更新
- **権威 DNS**: AWS Route53 のパブリックホストゾーン（2026-05 に外部 DNS から移行）
- **DNS 検証 CNAME の管理**: Route53 ホストゾーン上に CDK で登録
...
- ドメインが外部レンタルサーバー（XServer）で取得済みであること
- Route53 ホストゾーン（`NagiyuSharedRoute53` スタック）がデプロイ済みであること
