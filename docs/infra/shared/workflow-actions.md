...
- 内部で Docker ビルドロックの取得・解放を行います。ロック用バケットは dev/prod アカウントごとに別バケットのため、呼び出し元が明示指定しない限りはこの action が実行時の AWS アカウント ID から自動判定します（詳細は [Docker ビルド排他制御](./docker-build-lock.md) を参照）
