...
- 契約が固定してよいのは、**実 DynamoDB が仕様として保証している振る舞い**だけである。近似実装は決定的に振る舞う（挿入順で安定する等）ため、観測できた挙動をそのまま契約にすると、実 DynamoDB が保証していない順序まで仕様として固定してしまう。全件走査の結果順序が典型例で、これを契約にすると近似実装の都合を正としたことになる。
...
乖離は境界にしか現れないことが多い。既定ページサイズ、ちょうど使い切る件数、その倍数といった値である。件数を「多め」に積むだけでは足りない。実際に、既定 50 件に対して 130 件のフィクスチャを用意してもなお「ちょうど 50 件で終わる」境界が抜けており、後続のレビューで初めて検出された。
...
- **等価性を揃えると、呼び出し元の不具合が初めて顕在化することがある**。近似実装が実装より多く返していたために、本番では起きている打ち切りが E2E では再現していなかった、という形で現れる。等価性の担保と呼び出し元の修正はスコープが別なので、切り分けて扱う。
