...
- **結合テスト（上表）**: V字モデル上の**自動テストの階層**。本プラットフォームでは画面遷移・コンポーネント連携の多くを Playwright E2E が吸収しており、データアクセス層の等価性（InMemory と実 DynamoDB が一致するか）を確かめる契約テストが、この層の具体例にあたる。
- **dev 結合環境**: 実 AWS へデプロイして確認する**デプロイ先**であり、テスト階層ではない。担保対象は実インフラ差分（実 DynamoDB・実 auth・IAM・デプロイ/起動）に限る。

層ごとの責務分担と、E2E・契約テストの位置づけは [テスト戦略](./testing.md) を参照。
