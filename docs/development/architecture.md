...
複数環境（dev、prod等）を持つシステムでは、環境間で認証情報が混同される問題が発生する可能性がある。特に、同一のルートドメイン配下に複数環境がある場合、**認証フロー全体で使用されるすべてのクッキー**の管理が重要となる。

NextAuth v5 (Auth.js) は以下の複数のクッキーを使用するため、**すべてのクッキー**を環境別に分離する必要がある：
- セッショントークン（`sessionToken`）
- コールバックURL（`callbackUrl`）
- CSRFトークン（`csrfToken`）
- OAuthステート（`state`）
- PKCEコードベリファイア（`pkceCodeVerifier`）
- Nonce（`nonce`）
...
- 各環境で独立した認証状態を維持
- 環境間で**すべての認証関連クッキー**が混同されないようにする
- 各環境内ではSSO（Single Sign-On）を実現

**実装方針**:

- **すべてのクッキー名で環境を分離**: 環境ごとに異なるクッキー名を使用
...
- **完全な環境分離**: すべてのクッキー名が異なるため、dev環境とprod環境で認証フロー全体が混同されない
...
- NextAuth の設定で、`sessionToken` だけでなく、`callbackUrl`、`csrfToken`、`state`、`pkceCodeVerifier`、`nonce` もすべて環境別に設定する
- これを怠ると、一部のクッキーが環境間で混同され、認証フローが正常に動作しなくなる
...
- **すべてのクッキー**について、環境別の設定ロジックに対する単体テストを実装
- 環境判定ロジックが正しく動作することを確認

**運用時の検証**:

- 各環境でのログイン時に、**すべてのクッキー**が正しい名前とドメイン設定で適用されていることを確認
- 環境をまたいだログインで、認証状態が独立していることを確認
- ブラウザの開発者ツールでクッキーを確認し、環境別のサフィックスが正しく付いていることを検証

## 共通化の判断基準

コードを `libs/` に切り出すかどうかは、以下の基準で判断する。

### 切り出す条件

- **2サービス以上で同一実装が存在する**: 実質的に同一のコードが複数箇所にある場合、共通化を検討する
- **実装が独立して完結している**: 外部状態や特定サービスのコンテキストに依存せず、単体でテスト可能な場合
- **変更時の波及が問題になっている**: 同じ修正を複数箇所に適用しなければならない状況が発生した場合

### 切り出さない条件

- **1箇所にしか存在しない**: 現時点で利用箇所が1つの場合は、将来の再利用が見込まれない限り切り出さない
- **サービス固有のビジネスロジックを含む**: 特定サービスのドメイン知識に依存する処理は共通化しない
- **単なる薄いラッパー**: ライブラリ関数を1枚包むだけの関数は共通化ではなく冗長な間接層になる。元の関数を直接呼び出すこと
...
| AWS SDK に依存 | `libs/aws/` |
| サービス固有 UI コンポーネント | 各サービスの `core` パッケージ |
