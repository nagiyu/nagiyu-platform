...
1. **ディレクトリ名の決定**
    - `tasks/{feature-name}/` ディレクトリを作成
    - 例: `tasks/add-user-auth/`, `tasks/setup-cloudfront/`
    - Issue番号を含めても良い（`tasks/issue-123-auth/` 等）

2. **作業規模の判定**
    作業規模に応じて作成するファイルを決定（詳細は [tasks/templates/README.md](../../tasks/templates/README.md) を参照）:
...
3. **各ファイルの生成**
    ...
    - **`requirements.md`**: `docs/templates/services/requirements.md` をベースに、この作業スコープの要件を記述。冒頭に以下のコメントを追加:
        ```markdown
        <!--
            このドキュメントは開発時のみ使用します。
...
2. **必要に応じて詳細化**: `tasks/templates/` 配下のテンプレートを参照して各ファイルを補完
3. **実装エージェントの起動**: `task.implement`エージェントにドキュメントを渡して実装開始

## 参考リソース

- [task.implement エージェント](./task.implement.agent.md) - 実装を担当するエージェント
- [開発フロー](../../docs/development/flow.md) - 2層ドキュメント構成と開発フロー
- [tasks テンプレート](../../tasks/templates/README.md) - タスクドキュメントテンプレート一覧
