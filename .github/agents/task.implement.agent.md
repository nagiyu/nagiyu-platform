...
1. **全ドキュメントの読み込み** (ドキュメントが正):
    - **サービスの永続化ドキュメント構成** (`docs/services/{サービス名}/`):
        - `README.md` - 概要とドキュメント一覧
        - `requirements.md` - 要件定義（機能要件、非機能要件、ユースケース、ドメインオブジェクト）
        - `external-design.md` - 外部設計（画面設計・概念データモデル・設計上の決定事項）
        - `architecture.md` - アーキテクチャ設計決定記録（ADR）

    - **開発時ドキュメント構成** (`tasks/{feature-name}/`):
        - `requirements.md` - この作業スコープの要件定義（開発時のみ）
        - `external-design.md` - UI設計（UI変更がある場合・開発時のみ）
        - `design.md` - 技術設計（API仕様・データモデル・コンポーネント設計・開発時のみ）
        - `tasks.md` - 実装タスクリスト（開発時のみ）
...
2. **現在のフェーズとタスクの特定**:
    - `tasks/{feature-name}/tasks.md` から現在のフェーズと未完了タスクを確認
    - `tasks/{feature-name}/tasks.md` が存在しない場合、Issueから判断
...
2. **ドキュメント更新**:
    ...
    **ドキュメント配置ルール**:
    - **開発時**: `tasks/{feature-name}/` 配下の一時ドキュメントを参照・更新する（task.proposal が生成済み）
    - **永続化は対象外**: `docs/` への移行と `tasks/` 削除は `docs.write` エージェントの役割
    - **理由**: 各エージェントの責務を明確に分離する
...
3. **実装ワークフロー**:
    ...
    Phase 3: 検証とデプロイ
    3a. tasks/{feature-name}/tasks.md のチェックリストを更新
...
2. **技術スタック**:
    - `tasks/{feature-name}/design.md` または `tasks/{feature-name}/requirements.md` から技術スタックを確認
    - 各技術のベストプラクティスに従う
    - プロジェクト固有のコーディング規約を遵守

3. **アーキテクチャ**:
    - `tasks/{feature-name}/design.md` に定義されたアーキテクチャパターンに従う
    - ディレクトリ構造、モジュール分割などを遵守
    - 既存コードとの整合性を保つ

4. **型システム**:
    - TypeScript等の型システムを使用する場合、厳格な設定を使用
    - `tasks/{feature-name}/design.md` に従って型を定義
    - 型安全性を最優先

5. **インフラ**:
    - `tasks/{feature-name}/design.md` のインフラ設計に従う
...
1. **タスク進捗更新**:
    - `tasks/{feature-name}/tasks.md` で完了タスクを `[x]` としてマーク
    - `tasks/` が存在しない場合、Issueを更新
    - 必要に応じてステータスと日付を更新
    - ドキュメントを実際の進捗と同期
...
- （オプション）`tasks/{feature-name}/` 配下のドキュメントへの参照（全体像把握のため）
- 基本的なドキュメント構造が存在（最低限README.mdまたは要件定義）

**Note**:
- `tasks/{feature-name}/` は開発時のみ存在する一時的なドキュメントディレクトリ
- 開発完了後は永続化すべき内容を `docs/` に反映し、`tasks/{feature-name}/` を削除する
- 個別の実装はそれぞれ別Issueで対応
