...
describe("naming utilities", () => {
  describe("getResourceName", () => {
    it("should generate resource name with correct pattern", () => {
      expect(getResourceName("tools", "ecr", "dev")).toBe(
        "nagiyu-tools-ecr-dev",
      );
      expect(getResourceName("auth", "lambda", "prod")).toBe(
    ...
    it("should handle different resource types", () => {
      ...
      expect(getResourceName("tools", "dynamodb", "prod")).toBe(
        "nagiyu-tools-dynamodb-prod",
      );
    });
  });
  ...
  describe("getLambdaFunctionName", () => {
    it("should generate Lambda function name for dev environment", () => {
      expect(getLambdaFunctionName("tools", "dev")).toBe(
        "nagiyu-tools-lambda-dev",
      );
    });

    it("should generate Lambda function name for prod environment", () => {
      expect(getLambdaFunctionName("auth", "prod")).toBe(
        "nagiyu-auth-lambda-prod",
      );
    });
  });
  ...
  describe("getCloudFrontDomainName", () => {
    ...
    it("should generate CloudFront domain name for dev environment with prefix", () => {
      expect(getCloudFrontDomainName("tools", "dev")).toBe(
        "dev-tools.nagiyu.com",
      );
      expect(getCloudFrontDomainName("auth", "dev")).toBe(
        "dev-auth.nagiyu.com",
      );
    });
  });
  ...
  describe("getDynamoDBTableName", () => {
    it("should generate DynamoDB table name", () => {
      expect(getDynamoDBTableName("auth", "dev")).toBe(
        "nagiyu-auth-dynamodb-dev",
      );
      expect(getDynamoDBTableName("tools", "prod")).toBe(
        "nagiyu-tools-dynamodb-prod",
      );
    });
  });
  ...
  describe("getLogGroupName", () => {
    it("should generate CloudWatch Logs log group name", () => {
      expect(getLogGroupName("tools", "dev")).toBe(
        "/aws/lambda/nagiyu-tools-lambda-dev",
      );
      expect(getLogGroupName("auth", "prod")).toBe(
        "/aws/lambda/nagiyu-auth-lambda-prod",
      );
    });
  });
});
