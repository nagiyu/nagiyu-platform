      ...
      "summaries": [
        {
          ...
          "aiAnalysisError": "string | undefined"
        }
      ]
    }
  ]
...
| `undefined` の意味 | AI 解析が未生成（バッチ未実行等）または生成失敗（`aiAnalysisError` を参照）（UI: 「AI 解析はまだ生成されていません」と表示） |
| 言語 | 日本語 |
| 内容 | 価格動向の解釈、パターン分析の説明、関連市場情報を含む解析テキスト |
| 最大文字数 | 約 2,000 文字（OpenAI レスポンス依存） |

#### `summaries[].aiAnalysisError`

| 項目 | 値 |
|------|---|
| 型 | `string` または `undefined` |
| `string` の意味 | AI 解析の生成が失敗した際のエラー情報（UI: 「AI 解析の取得に失敗しました」と表示） |
| `undefined` の意味 | エラーなし（未生成または生成成功） |
...
| バージョン | 変更内容 |
|-----------|---------|
| 2.0.0 | `summaries[].aiAnalysis` / `summaries[].aiAnalysisError` フィールドを追加（optional） |
| 1.0.0 | 初版（既存実装） |
