...
- [ ] T003 [P] `services/stock-tracker/web/types/stock.ts` に `PatternDetail` インターフェースを追加し、`TickerSummary` に `buyPatternCount`・`sellPatternCount`・`patternDetails` フィールドを追加する（contracts/api.md の型定義に準拠）
...
- [ ] T012 [US1] `services/stock-tracker/web/app/api/summaries/route.ts` を変更してパターン情報（`buyPatternCount`・`sellPatternCount`・`patternDetails`）をレスポンスに含め、`services/stock-tracker/web/tests/unit/app/api/summaries/route.test.ts` にユニットテストを追加する（バッチ未実行アイテムはデフォルト値 `{ buyPatternCount: 0, sellPatternCount: 0, patternDetails: [] }` を返す）
