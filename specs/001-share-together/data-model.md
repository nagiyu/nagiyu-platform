...
| `memberCount` | Number | ✅ | メンバー数（オーナー含む） |
| `createdAt` | String | ✅ | ISO 8601 形式 |
| `updatedAt` | String | ✅ | ISO 8601 形式 |
...
| キー | 値 |
|------|----|
| `PK` | `USER#{inviteeUserId}` |
| `SK` | `INVITE#{inviteId}` |
| `TTL` | Unix 秒（30日後に自動削除） |

#### 属性

| フィールド | 型 | 必須 | 説明 |
|-----------|-----|------|------|
| `inviteId` | String | ✅ | 招待 ID（UUID） |
| `inviteeUserId` | String | ✅ | 招待されたユーザー ID |
| `groupId` | String | ✅ | 招待対象グループ ID |
| `groupName` | String | ✅ | グループ名（表示用） |
| `inviterUserId` | String | ✅ | 招待者のユーザー ID |
...
| パターン | 操作 | キー条件 |
|---------|------|---------|
| ユーザーの保留中招待一覧取得 | Query | `PK = USER#{userId}`, `SK begins_with INVITE#` |
| 特定の招待取得 | GetItem | `PK = USER#{userId}`, `SK = INVITE#{inviteId}` |

---

### 6. グループ共有 ToDo リスト (GroupList)
...
| 11 | グループの共有リスト一覧 | Query | メインテーブル | `PK=GROUP#{groupId}`, `SK begins_with GLIST#` |
| 12 | 特定共有リスト取得 | GetItem | メインテーブル | `PK=GROUP#{groupId}`, `SK=GLIST#{listId}` |
| 13 | ユーザーへの保留中招待一覧 | Query | メインテーブル | `PK=USER#{userId}`, `SK begins_with INVITE#` |
| 14 | 特定招待通知取得 | GetItem | メインテーブル | `PK=USER#{userId}`, `SK=INVITE#{inviteId}` |
