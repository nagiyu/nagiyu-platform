...
- 1 ユーザーが持てる個人リストの総数はデフォルトリストを含め最大 100 件（FR-005）
...
- グループ名の変更はオーナー（`ownerUserId` と一致するユーザー）のみ可能（FR-022）
...
| `TTL` | Number | | Unix エポック秒。`PENDING` 状態のレコードにのみ設定（招待日時 + 86400 秒）。承認・拒否時に削除またはクリアする |
| `createdAt` | String | ✅ | ISO 8601 形式 |
| `updatedAt` | String | ✅ | ISO 8601 形式 |
...
- 1 グループのメンバー（`ACCEPTED` + `OWNER`）はオーナーを含め最大 5 名（FR-023）
- `PENDING` レコードには `TTL` を設定し、招待日時から 1 日（86400 秒）で自動失効させる（FR-024）。承認・拒否時は `TTL` を削除またはクリアする
