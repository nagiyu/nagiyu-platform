...
- [ ] T001 `services/share-together/core/` パッケージ全体を作成（`package.json` @nagiyu/share-together-core、`tsconfig.json`、`eslint.config.mjs`、`jest.config.ts`（coverageThreshold 80%）、`src/index.ts` プレースホルダー）
- [ ] T002 `services/share-together/web/` パッケージ全体を作成（`package.json` @nagiyu/share-together-web、`tsconfig.json`、`eslint.config.mjs`、`jest.config.ts`、`playwright.config.ts`（chromium-desktop / chromium-mobile / webkit-mobile）、`next.config.ts`（standalone 出力）、`Dockerfile`（マルチステージビルド）、`src/app/layout.tsx` / `page.tsx` プレースホルダー）
- [ ] T003 Verify CI ワークフローを2件作成（`.github/workflows/share-together-web-verify-fast.yml`：`integration/**` PR でトリガー、lint / format-check / build-core / build-web / docker-build / test-core / e2e-test chromium-mobile / synth-infra。`.github/workflows/share-together-web-verify-full.yml`：`develop` PR でトリガー、Fast CI 全ジョブ + カバレッジ 80% チェック + E2E 全デバイス）
...
- [ ] T006 [P] PWA アセットを作成（`services/share-together/web/public/manifest.json`（アプリ名・アイコン・`display: standalone`）、`services/share-together/web/public/sw.js`（キャッシュ戦略のみ、プッシュ通知コードはコメントアウト）、`services/share-together/web/src/components/ServiceWorkerRegistration.tsx`）
